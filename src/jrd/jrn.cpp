...
/* FREE: by error returns from this module, otherwise by JRN_fini & friends */
	...
	for (int loop = 0; loop < 20; loop++) {
		...
		if (!connect((long) journal->jrn_channel, (sockaddr*)&address, sizeof(address)))
			break;

		sleep(3);

		if (loop < 16) {
			close((long) journal->jrn_channel);
			continue;
		}
		if (!SYSCALL_INTERRUPTED(errno)) {
			if (retry) {
...
/*  If the recv call is interrupted, another process is in contention
	...
	do {
		l = recv((long) journal->jrn_channel, (char *) reply, sizeof(struct jrnr), 0);
...
#ifdef BSD_SOCKETS
	if (close((long) journal->jrn_channel) < 0) {
...
/*  If the send call is interrupted, another process is in contention
	...
	do {
		l = send((long) journal->jrn_channel, (char *) buffer, (int) length, 0);
	} while (l < 0 && SYSCALL_INTERRUPTED(errno));
