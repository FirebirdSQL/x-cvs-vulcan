...
static int get_dbkey(TDBB tdb, RSB);
static int get_indexed(TDBB tdb, RSB);
...
static int open_indexed(TDBB tdb, RSB);
static int open_sequential(TDBB tdb, RSB);
...
EXT EXT_file(tdbb *tdbb, REL relation, TEXT * file_name, SLONG * description)
{
/**************************************
 *
 *	E X T _ f i l e
...
RSB EXT_optimize(TDBB tdb, OPT opt, SSHORT stream, NOD * sort_ptr)
{
/**************************************
 *
   ...
   against, go at it */
	...
	if (!dbkey && opt->opt_count && (idx = file->ext_indices))
		for (i = 0; i < file->ext_index_count; i++) {
			if (OPT_match_index(tdbb, opt, stream, idx) &&
...
static get_dbkey(TDBB tdb, RSB rsb)
{
/**************************************
 *
 *	g e t _ d b k e y
...
/* Chase down misc pointers */
	...
	impure = IMPURE (request, rsb->rsb_impure);
	rpb = &request->req_rpb[rsb->rsb_stream];
	record = rpb->rpb_record;
	format = record->rec_format;
...
static get_indexed(TDBB tdb, RSB rsb)
{
/**************************************
 *
...
static BOOLEAN get_sequential(TDBB tdb, RSB rsb)
{
/**************************************
 *
...
static open_indexed(TDBB tdb, RSB rsb)
{
/**************************************
 *
...
static open_sequential(TDBB tdb, RSB rsb)
{
/**************************************
 *
