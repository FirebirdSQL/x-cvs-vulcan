...
static FIL seek_file(FIL, BDB, OVERLAPPED*, OVERLAPPED**);
static FIL setup_file(DBB, const TEXT*, USHORT, HANDLE);
static BOOLEAN nt_error(TEXT*, const fil*, ISC_STATUS);
 ...
 **************************************/
	...
	if (ostype == OS_CHICAGO)
	{
		...
		{
			THD_MUTEX_UNLOCK(file->fil_mutex);
			nt_error("SetFilePointer", file, isc_io_read_err);
		}
		overlapped_ptr = NULL;
	}
	else
 ...
 **************************************/
	...
	if (!(file = seek_file(file, bdb, &overlapped, &overlapped_ptr)))
		return FALSE;
	...
	if (!(piob->piob_flags & PIOB_success)) {
		...
		if (!GetOverlappedResult((HANDLE) piob->piob_desc,
			...
			release_io_event(piob->piob_file,
							 (OVERLAPPED *) & piob->piob_io_event);
			return nt_error("GetOverlappedResult", piob->piob_file, isc_io_error);
		}
	}
 ...
 **************************************/
	...
	file = seek_file(file, bdb, &overlapped, &overlapped_ptr);
	...
	if (dbb->dbb_encrypt_key) 
		...
		if (!WriteFile(desc, spare_buffer, size, &actual_length, overlapped_ptr) || actual_length != size)
			{
			if (ostype == OS_CHICAGO) 
				THD_MUTEX_UNLOCK(file->fil_mutex);
