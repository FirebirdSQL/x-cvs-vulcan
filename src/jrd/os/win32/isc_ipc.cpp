 ...
 **************************************/
	...
	for (; opn_event < end_opn_event; opn_event++) 
		{
		if (opn_event->opn_event_pid == pid && opn_event->opn_event_signal == signal_number) 
			break;
	...
	int ret = SetEvent(opn_event->opn_event_lhandle);
 ...
 **************************************/
	...
	system_overflow_handler = signal(SIGFPE, reinterpret_cast<void(*)(int)>(overflow_handler));
