...
static int dump(ISC_QUAD*, isc_handle, isc_handle, IB_FILE*);
static int edit(ISC_QUAD*, isc_handle, isc_handle, SSHORT, const SCHAR*);
static int get_ods_version(isc_handle*, USHORT*, USHORT*);
static void isc_expand_dpb_internal(const UCHAR** dpb, SSHORT* dpb_size, ...);
static int load(ISC_QUAD*, isc_handle, isc_handle, IB_FILE*);
static void printVersion (void *arg, const char *text);
...
BOOLEAN API_ROUTINE isc_set_path(const TEXT* file_name,
 ...
 **************************************/
	...
	if (!routine) 
		{
		routine = printVersion;	//(FPTR_VERSION_CALLBACK) ib_printf;
		user_arg = (void*) "\t%s\n";
		}
	...
	while (count-- > 0) 
		...
		if (class_ >= FB_NELEM(impl_class) || !(class_string = impl_class[class_]))
...
BSTREAM* API_ROUTINE BLOB_open(isc_handle blob, SCHAR* buffer, int length)
{
/**************************************
 *
 *	B L O B _ o p e n
...
void printVersion (void *arg, const char *text)
{
	ib_printf ((const char*) arg, text);
}
