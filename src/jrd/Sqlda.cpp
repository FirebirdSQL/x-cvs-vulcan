...
void Sqlda::allocBuffer()
{
	...
	for (n = 0; n < numberColumns; ++n, ++var)
	{
		...
		if (length == 0)
			throw SQLError (COMPILE_ERROR, "Sqlda variable has zero length");
...
void Sqlda::setValue(int slot, Value * value, InternalConnection *connection)
{
	...
	*var->sqlind = 0;
	...
	switch (value->type)
		...
		case Date:
			...
			*(ISC_DATE*)var->sqldata = InternalStatement::getIscDate (value->data.date);
			break;
									
		case TimeType:
			var->sqltype = SQL_TYPE_TIME;
			var->sqllen = sizeof (ISC_TIME);
			*(ISC_TIME*)var->sqldata = InternalStatement::getIscTime (value->data.time);
			break;
									
		case Timestamp:
			var->sqltype = SQL_TIMESTAMP;
			var->sqllen = sizeof (ISC_TIMESTAMP);
			*(ISC_TIMESTAMP*)var->sqldata = InternalStatement::getIscTimeStamp (value->data.timestamp);
			break;
...
void Sqlda::setBlob(XSQLVAR * var, Value * value, InternalConnection *connection)
{
	if (value->type == Null)
	{
		var->sqltype |= 1;
...
void WriteToArray(InternalConnection *connect,XSQLVAR *var,Value * value);

void Sqlda::setArray(XSQLVAR * var, Value * value, InternalConnection *connection)
{
	if (value->type == Null)
		{
		var->sqltype |= 1;
