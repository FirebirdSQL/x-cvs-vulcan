...
static void apply_data(TDBB tdbb, DPG, jrnd*);
static void apply_header(HDR, jrnd*);
static void apply_ids(PPG, jrnd*);
static void apply_index(TDBB tdbb, BTR, jrnd*);
...
static void rec_process_record(TDBB, JRNH*, USHORT, ULONG,
							   ULONG, PAG, SBM*, bool);
static void scan_and_apply_logs(TDBB tdbb, const TEXT*, TEXT*, CP*, SBM*, bool, SLONG*,
								SLONG, PAG);
 ...
 **************************************/
	...
	scan_and_apply_logs(tdbb, dbname, walname, cp1, sbm_rec,
						activate_shadow, NULL, -1, NULL);
	REC_update_next_transid(tdbb);
...
/* get current time stamp */
	...
	scan_and_apply_logs(tdbb, dbname, walname, cp1, NULL,
						false, date, page_no, page);
}


void REC_update_next_transid(TDBB tdbb)
{
/**************************************
 *
...
static void apply_data(TDBB tdbb, DPG page, jrnd* record)
{
/**************************************
 *
 *	a p p l y _ d a t a
...
static void apply_index(TDBB tdbb, BTR page, jrnd* record)
{
/**************************************
 *
...
/* changes need to be applied */
	...
	case pag_data:
		apply_data(tdbb, reinterpret_cast < dpg * >(page), record);
		break;
	...
	case pag_root:
		...
		break;
	case pag_index:
		apply_index(tdbb, reinterpret_cast < btr * >(page), record);
		break;
	case pag_ids:
...
static void scan_and_apply_logs(TDBB tdbb,
								const TEXT* dbname,
								TEXT* start_logname,
								CP* cp1,
								SBM* sbm_rec,
