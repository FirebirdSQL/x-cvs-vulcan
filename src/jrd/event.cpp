...
SLONG EVENT_create_session(ISC_STATUS * status_vector, Database *database)
{
/**************************************
 *
...
// If we're not initialized, do so now.
	...
	if (!EVENT_header && !EVENT_init(status_vector, database, TRUE))
		return 0;
...
EVH EVENT_init(ISC_STATUS * status_vector, Database *database, USHORT server_flag)
{
/**************************************
 *
 *	E V E N T _ i n i t
 ...
 **************************************/
	...
	EVENT_default_size = database->configuration->getValue (EventMemSize,EventMemSizeValue); //Config::getEventMemSize();
 ...
 **************************************/
	...
	if (!EVENT_header && !EVENT_init(status_vector, database, FALSE))
 ...
 **************************************/
	...
	while (p < end) 
		...
		if (interest = historical_interest(session, event_offset)) 
			{
			for (PTR* ptr2 = &session->ses_interests; *ptr2 && (prior = (RINT) ABS_PTR(*ptr2));
 ...
 **************************************/
	...
	if (EVENT_header->evh_length > EVENT_data.sh_mem_length_mapped)
		...
		AsyncEvent* event = process->prb_event;
...
/* Untangle and release process block */
	...
	if (EVENT_process_offset == process_offset) {
		...
		while (process->prb_flags & PRB_exiting && !timeout) {
			...
			RELEASE;
#ifdef SOLARIS_MT
			AsyncEvent* events = EVENT_process->prb_event;
#else
			AsyncEvent* events = process->prb_event;
#endif
 ...
 **************************************/
	...
	*p++ = EPB_version1;
	...
	for (PTR next = request->req_interests; next && (interest = (RINT) ABS_PTR(next));
 ...
 **************************************/
	...
	sprintf(msg, "AsyncEvent*: %s error, status = %d", string, mutex_state);
	gds__log(msg);
 ...
 **************************************/
	...
	ib_printf("(AsyncEvent*) punt: global region corrupt -- %s\n", string);
}
