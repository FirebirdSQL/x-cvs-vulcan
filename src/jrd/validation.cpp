...
IV. PHASES OF VALIDATION
   ...
   B. Garbage Collection
      ...
      During this phase, the Page Inventory (page_inv_page*) pages are checked against the
      ...
      be made to the page_inv_page* pages.  This assumes that we did not have a chance to
...
V. WALK-THROUGH PHASE
   ...
   A. Page Fetching
      ...
      4. Transaction Inventory pages
         ...
         contain any tx_inv_page* pages, the message: 
         ...
         If -mend is specified, then a new tx_inv_page* will be allocated on disk and 
         stored in RDB$PAGES in the proper sequence.  All transactions which 
         would have been on that page are assumed committed.
 
         If a tx_inv_page* page does not point to the next one in sequence, the
...
static RTN corrupt(thread_db*, VDR, USHORT, Relation*, ...);
...
#endif
static RTN walk_blob(thread_db*, VDR, Relation*, BLH, USHORT, SLONG);
static RTN walk_chain(thread_db*, VDR, Relation*, RHD, SLONG);
static void walk_database(thread_db*, VDR);
static RTN walk_data_page(thread_db*, VDR, Relation*, SLONG, SLONG);
static void walk_generators(thread_db*, VDR);
static void walk_header(thread_db*, VDR, SLONG);
static RTN walk_index(thread_db*, VDR, Relation*,
					  index_root_page& root_page, USHORT id);
static void walk_log(thread_db*, VDR);
static void walk_pip(thread_db*, VDR);
static RTN walk_pointer_page(thread_db*, VDR, Relation*, int);
static RTN walk_record(thread_db*, VDR, Relation*, RHD, USHORT, SLONG, bool);
static RTN walk_relation(thread_db*, VDR, Relation*);
static RTN walk_root(thread_db*, VDR, Relation*);
static RTN walk_tip(thread_db*, VDR, SLONG);

static const SLONG end_level = END_LEVEL, end_bucket = END_BUCKET;

BOOLEAN VAL_validate(thread_db* tdbb, USHORT switches)
{
/**************************************
 *
 ...
 **************************************/
	struct vdr control;
	JrdMemoryPool* val_pool = NULL;
	JrdMemoryPool* old_pool = NULL;
	...
	try 
		...
		val_pool = 0;
		tdbb->tdbb_default = val_pool = JrdMemoryPool::createPool(tdbb->tdbb_database);
...
static RTN corrupt(thread_db* tdbb, VDR control, USHORT err_code, Relation* relation, ...)
{
/**************************************
 *
 *	c o r r u p t
 ...
 **************************************/
	...
	*(PAG *) page_pointer = CCH_FETCH_NO_SHADOW(tdbb, window, LCK_write, 0);
	...
	if ((dbb->dbb_flags & DBB_damaged) && !dbb->pageCache->validate(window)) 
		{
	...
	PBM_SET(tdbb->tdbb_default, &control->vdr_page_bitmap, page_number);
...
static RTN walk_blob(thread_db* tdbb, VDR control, Relation* relation, 
					 BLH header, USHORT length, SLONG number)
{
/**************************************
 *
 *	w a l k _ b l o b
...
static RTN walk_chain(thread_db* tdbb, VDR control,
					  Relation* relation, RHD header, SLONG head_number)
{
/**************************************
 *
 *	w a l k _ c h a i n
 ...
 **************************************/
	...
	for (int i = 0; i < dbb->dbb_relations.size(); ++i)
		{
		Relation* relation = dbb->dbb_relations[i];
...
static RTN walk_data_page(thread_db* tdbb,
						  VDR control,
						  Relation* relation, SLONG page_number, SLONG sequence)
{
/**************************************
 *
		...
		if (line->dpg_length) 
			...
			if ((control->vdr_flags & vdr_records) 
				...
				if (header->rhd_b_page)
					RBM_SET(tdbb->tdbb_default, &control->vdr_rel_records, number);
				else 
					{
 ...
 **************************************/
	...
	for (int n = 0; n < dbb->dbb_gen_id_pages.size(); ++n)
		{
		SLONG pageNumber = dbb->dbb_gen_id_pages[n];
...
#endif
			fetch_page(tdbb, control, pageNumber, pag_ids, &window, &page);
			CCH_RELEASE(tdbb, &window);
			}
		}
}
...
static RTN walk_index(thread_db* tdbb, VDR control, Relation* relation, 
					  index_root_page& root_page, USHORT id)
{
/**************************************
 *
	...
	while (next) 
		...
		while (pointer < endPointer) 
			...
			if (leafPage && control && (control->vdr_flags & vdr_records))
				RBM_SET(tdbb->tdbb_default, &control->vdr_idx_records, node.recordNumber.getValue());
 ...
 **************************************/
	...
	for (pages = page->ppg_page, slot = 0; slot < page->ppg_count; slot++, pages++, seq++)
		if (*pages) 
			{
 ...
 **************************************/
	...
	Format* format = relation->getFormat(tdbb, header->rhd_format);
...
static RTN walk_relation(thread_db* tdbb, VDR control, Relation* relation)
{
/**************************************
 *
 ...
 **************************************/
	...
	try 
		...
		{
		TEXT s[64];
		const char* msg = ((bool) relation->rel_name) ?
			"bugcheck during scan of table %d (%s)" :
			"bugcheck during scan of table %d";
		sprintf(s, msg, relation->rel_id, (const char*) relation->rel_name);
		gds__log(s);
#ifdef DEBUG_VAL_VERBOSE
		if (VAL_debug_level)
			ib_fprintf(ib_stdout, "LOG:\t%s\n", s);
...
static RTN walk_root(thread_db* tdbb, VDR control, Relation* relation)
{
/**************************************
 *
 *	w a l k _ r o o t
 ...
 **************************************/
	...
	***/
	...
	for (ULONG sequence = 0; sequence <= pages; sequence++) 
		...
		WIN window(-1);
		fetch_page(	tdbb, control, dbb->dbb_t_pages[sequence],
					pag_transactions, &window, &page);
...
#endif
		if (page->tip_next && page->tip_next != dbb->dbb_t_pages[sequence + 1])
