...
static MOD search_for_module(Database *database, const TEXT *, const TEXT *, bool);
...
#ifdef VMS
#define LOOKUP
FPTR_INT ISC_lookup_entrypoint(Database *database,
							   TEXT* module,
							   TEXT* name,
 ...
 **************************************/
	...
	if (!mod)
		...
		TEXT absolute_module[MAXPATHLEN];
		strcpy(absolute_module, module);
		int length = (int) strlen(absolute_module);
...
#define OPEN_HANDLE(name) shl_load(name, BIND_DEFERRED, 0)
 ...
 **************************************/
	...
	if (!mod)
        ...
        if (!mod) 
			{  // Start looking for "libxxxx.so" module names
            JString moduleName = "lib";
            moduleName += absolute_module;
			mod = search_for_module(database, moduleName, name, ShowAccessError);
			}
...
#define OPEN_HANDLE(name) dlopen(name, RTLD_LAZY)
 ...
 **************************************/
	...
	if (!mod)
		...
		int length = strlen(absolute_module);
...
HMOD AdjustAndLoad(JString name) 
{
/**************************************
 *
 ...
 **************************************/
	/***
	NAMESPACE::string suffix = name.length() > 4 ?
		name.substr(name.length() - 4, 4) : NAMESPACE::string("");
...
static MOD search_for_module(Database *database, const TEXT* module_name, const TEXT* name, bool ShowAccessError)
{
/**************************************
 *
 ...
 **************************************/
	...
	static class UdfDirectoryList : public DirectoryList 
		{
		const NAMESPACE::PathName GetConfigString(const char *configString) const 
			{
			return NAMESPACE::PathName(Config::getUdfAccess());
			}
		} iUdfDirectoryList;
	***/
	
	//JString option = database->configuration->getValue (UdfAccess,UdfAccessValue);
	//JString paths = database->configuration->getValue (1, UdfAccess);
	
	try
		{
		JString option = database->configuration->getConcatenatedValues (UdfAccess);
		
		if (option.IsEmpty())
			option = UdfAccessValue;
			
		DirectoryList directoryList (option);

		/***
		NAMESPACE::PathName path, relative;
		NAMESPACE::PathName absolute_module = module_name;
