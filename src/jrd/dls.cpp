...
static MDLS DLS_cfg_tmpdir = { NULL };	/* directory list object */
 ...
 **************************************/
	...
	ptr = DLS_get_access(sfb->configuration);


	if (!sfb->sfb_dls) 
		{
		/* allocate temp. space starting search from the begining of the dls list */
		for (sfb->sfb_dls = ptr->mdls_dls; sfb->sfb_dls; sfb->sfb_dls = sfb->sfb_dls->dls_next)
 ...
 **************************************/
	...
	if (configuration && !is_initialized) 
		...
		***/
		...
		JString tempDirectories = configuration->getConcatenatedValues (TempDirectories);

		if (tempDirectories.IsEmpty())
			{
			char tempName [MAXPATHLEN];
			gds__temp_dir(tempName);
			tempDirectories = tempName;
			}
			
		for (const char *p = tempDirectories; *p;)
			{
			int size = 0;
			char directory [MAXPATHLEN], *q;
			for (q = directory; *p && *p != ' ' && *p != ';';)
				*q++ = *p++;
			*q = 0;
			while (*p == ' ')
				++p;
			...
			if (size == 0)
				size = MAX_TEMPFILE_SIZE;
				
			DLS_add_dir (size, directory);
			while (*p == ';')
				++p;
			}
		}
