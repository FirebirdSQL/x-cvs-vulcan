 ...
 **************************************/
	...
	if (status_vector[0] != isc_arg_gds ||
		...
		MOVE_FASTER(tmp_status, status_vector, sizeof(ISC_STATUS) * tmp_status_len);
		DEBUG;
		ERR_punt();
		}
	...
	if ((i = err_status_len + tmp_status_len) < ISC_STATUS_LENGTH) 
		...
		if (warning_count && i + warning_count - 1 < ISC_STATUS_LENGTH) 
			MOVE_FASTER(warning_status, &status_vector[i - 1], sizeof(ISC_STATUS) * warning_count);
		}
 ...
 **************************************/
	...
	if (dbb && (dbb->dbb_flags & DBB_bugcheck))
		gds__log_status((const char*) tdbb->tdbb_attachment->att_filename,
			tdbb->tdbb_status_vector);

	//Firebird::status_exception::raise(tdbb->tdbb_status_vector[1]);
	throw OSRIException (osriAlreadyStuffed);
}
#endif
