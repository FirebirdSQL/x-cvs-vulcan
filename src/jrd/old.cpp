...
static int close_cur_file(TDBB tdbb, OLD, USHORT);
static int create_file(TDBB tdbb, OLD, SLONG *);
static SLONG old_dump_all_pages(OLD, PIP, SLONG, ULONG);
static int old_dump_page(OLD, ULONG);
static void old_fini(TDBB tdbb, OLD *, USHORT);
 ...
 **************************************/
	...
	PAG_get_clump(tdbb, HEADER_PAGE, HDR_backup_info,
				  reinterpret_cast < USHORT * >(&d_len),
				  reinterpret_cast < UCHAR * >(data));
	PAG_get_clump(tdbb, HEADER_PAGE, HDR_journal_server,
...
/* dump all 'in use' pages in database.  */
	...
	for (sequence = 0;; sequence++) {
		...
		page = (PIP) dbb->pageCache->fetch(NULL, &window, LCK_read, pag_pages, DEFAULT_FETCH);
		MOVE_FAST((UCHAR *) page, (UCHAR *) temp_page, dbb->dbb_page_size);
		dbb->pageCache->release(NULL, &window);
	...
	if (last_page) {
		...
		old_fini(tdbb, &OLD_handle, OLD_EOF);
...
/* Add record to JOURNAL SERVER */
	...
	old_fini(tdbb, &OLD_handle, OLD_EOD);

	return FB_SUCCESS;
}


static int close_cur_file(TDBB tdbb, OLD old, USHORT code)
{
/**************************************
 *
...
static int create_file(TDBB tdbb, OLD old, SLONG * ret_fd)
{
/**************************************
 *
 ...
 **************************************/
	...
	dbb->pageCache->fetch(NULL, &window, LCK_read, pag_undefined, NO_CHKSUM);
...
static void old_fini(TDBB tdbb, OLD * OLD_handle, USHORT code)
{
/**************************************
 *
 *	o l d  _ f i n i
...
/* write a OLD_EOD record */
	...
	if ( (old = *OLD_handle) ) {
		if (old->old_fd > 0) {
			if (close_cur_file(tdbb, old, code) == FB_FAILURE)
				return;
		}
 ...
 **************************************/
	...
	if (old->old_fd > 0) {
		if (close_cur_file(tdbb, old, OLD_EOF) == FB_FAILURE)
			return FB_FAILURE;
	}
	...
	if (create_file(tdbb, old, &fd) == FB_FAILURE)
		return FB_FAILURE;
