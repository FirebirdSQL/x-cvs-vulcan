...
#define FREE(block)		delete [] block
	...
	UCHAR* target, size_t target_size)
{
	...
	if (origin_size > target_size) {
		temp_sdl = (UCHAR*)ALLOC(origin_size);
		...
		case isc_sdl_struct:
			for (n = *sdl++; n; --n)
			{
				if (*sdl == blr_d_float)
				{
					if (new_sdl == old_sdl)
					{
						UCHAR* temp_sdl = (UCHAR*)ALLOC(sdl_length);
...
SSHORT get_word(const UCHAR*& ptr)
{
/**************************************
 *
 * g e t _ w o r d
...
void SDL_free(const void *block)
{
/**************************************
 *
 *	S D L _ f r e e
 ...
 *
 * Functional description
 *	Release something allocated here.
 *
 **************************************/
