...
static UINT64 calculate_priority_level(thread_db* tdbb, OptimizerBlk*, index_desc*);
static void check_indices(thread_db* tdbb, csb_repeat *);
static bool check_relationship(OptimizerBlk*, USHORT, USHORT);
...
static USHORT distribute_equalities(thread_db* tdbb, LLS *, CompilerScratch*, USHORT);
...
#endif
static void find_best(thread_db*, OptimizerBlk*, USHORT, USHORT, UCHAR *, JRD_NOD,
					  double, double);
static JRD_NOD find_dbkey(JRD_NOD, USHORT, SLONG *);
static USHORT find_order(thread_db*, OptimizerBlk*, UCHAR *, JRD_NOD);
static void find_rsbs(RecordSource*, LLS *, LLS *);
static void find_used_streams(RecordSource* , UCHAR *);
...
static void get_inactivities(CompilerScratch*, ULONG *);
static IndexedRelationship* indexed_relationship(thread_db*, OptimizerBlk*, USHORT);
static STR make_alias(thread_db*, CompilerScratch*, csb_repeat *);
static JRD_NOD make_binary_node(thread_db* tdbb, NOD_T, JRD_NOD, JRD_NOD, bool);
static RecordSource* make_cross(thread_db*, OptimizerBlk*, LLS);
static JRD_NOD make_index_node(thread_db*, Relation*, CompilerScratch*, index_desc*);
...
static bool river_reference(River*, JRD_NOD, bool *field_found = NULL);
static bool search_stack(JRD_NOD, LLS);
static void set_active(OptimizerBlk*, River*);
static void set_direction(JRD_NOD, JRD_NOD);
static void set_inactive(OptimizerBlk*, River*);
static void set_made_river(OptimizerBlk*, River*);
static void set_position(JRD_NOD, JRD_NOD, JRD_NOD);
static void set_rse_inactive(CompilerScratch*, RecordSelExpr*);
static void sort_indices_by_selectivity(thread_db* tdbb, csb_repeat *);
static SSHORT sort_indices_by_priority(thread_db* tdbb, csb_repeat *, index_desc**, UINT64 *);
...
BOOLEAN OPT_access_path(thread_db* tdbb, const jrd_req* request,
						UCHAR* buffer,
 ...
 **************************************/
	...
	for (i = vector->count() - 1; i >= 0; i--) 
		{
		RecordSource* rsb = (RecordSource*) (*vector)[i];
...
/* Do not allocate the index_desc struct. Let BTR_all do the job. The allocated
   ...
   gets cleaned up before this function exits. */
	...
	opt_ = FB_NEW(*tdbb->tdbb_default) OptimizerBlk(tdbb->tdbb_default);
		 ...
		 ptr++)
	{
		...
		if (node->nod_type == nod_union) {
			...
			local_streams[++local_streams[0]] =
				(UCHAR)(long) node->nod_arg[e_uni_stream];
		}
			...
			fb_assert(local_streams[0] < MAX_STREAMS && local_streams[0] < MAX_UCHAR);
			local_streams[++local_streams[0]] =
				(UCHAR)(long) node->nod_arg[e_agg_stream];
		}
		else if (node->nod_type == nod_procedure) {
			rsb = gen_procedure(tdbb, opt_, node);
			fb_assert(local_streams[0] < MAX_STREAMS && local_streams[0] < MAX_UCHAR);
			local_streams[++local_streams[0]] =
				(UCHAR)(long) node->nod_arg[e_prc_stream];
		}
		...
		if (rsb) {
			i = local_streams[0];
			river = FB_NEW_RPT(*tdbb->tdbb_default, i) River();
...
JRD_NOD OPT_make_dbkey(thread_db* tdbb, OptimizerBlk* opt_, JRD_NOD boolean, USHORT stream)
{
/**************************************
 *
...
JRD_NOD OPT_make_index(thread_db* tdbb, OptimizerBlk* opt_, Relation* relation, index_desc* idx)
{
/**************************************
 *
 ...
 **************************************/
	...
	OptimizerBlk::opt_segment *tail, *end;
...
int OPT_match_index(thread_db* tdbb, OptimizerBlk* opt, USHORT stream, index_desc* idx)
{
/**************************************
 *
 ...
 **************************************/
	...
	JRD_NOD node;
	USHORT n;
	OptimizerBlk::opt_conjunct *tail, *opt_end;
...
/* set up a dummy optimizer block just for the purposes 
   of the set index, to pass information to subroutines */
	...
	opt = FB_NEW(*tdbb->tdbb_default) OptimizerBlk(tdbb->tdbb_default);
   ...
   structure, and vice versa to convert the other way */
	...
	if (idx) {
		...
		if (old_rsb->rsb_type == rsb_navigate)
			...
			new_rsb->rsb_impure += sizeof(struct impure_inversion);
		}
	}
	else if (old_rsb->rsb_type == rsb_navigate) {
		new_rsb->rsb_impure -= sizeof(struct impure_inversion);
	...
	if (idx && (index = CMP_get_index_lock(tdbb, relation, idx->idx_id)) ) 
		{
		if (!index->idl_count) 
...
static UINT64 calculate_priority_level(thread_db* tdbb, OptimizerBlk* opt, index_desc* idx)
{
/**************************************
 *
 ...
 *         
 **************************************/
	OptimizerBlk::opt_segment *idx_tail, *idx_end;
	...
	if (opt->opt_segments[0].opt_lower || opt->opt_segments[0].opt_upper) {
		...
		UINT64 max_idx = tdbb->tdbb_database->dbb_max_idx + 1;
...
static void check_indices(thread_db* tdbb, csb_repeat * csb_tail)
{
/**************************************
 *
...
static bool check_relationship(OptimizerBlk* opt, USHORT position, USHORT stream)
{
/**************************************
 *
 ...
 **************************************/
	...
	OptimizerBlk::opt_stream *tail, *end;
...
static void clear_bounds(OptimizerBlk* opt, index_desc* idx)
{
/**************************************
 *
 ...
 *
 **************************************/
	OptimizerBlk::opt_segment *tail, *opt_end;
...
static JRD_NOD compose(thread_db* tdbb, JRD_NOD * node1, JRD_NOD node2, NOD_T node_type)
{
/**************************************
 *
 *	c o m p o s e
...
/* Set sub-streams of rse active */
	...
	for (ptr = rse->rse_relation, end = ptr + rse->rse_count; ptr < end; ptr++) {
		if ((*ptr)->nod_type != nod_rse) {
			n = (USHORT)(long) (*ptr)->nod_arg[STREAM_INDEX((*ptr))];
...
static void compute_dbkey_streams(CompilerScratch* csb, JRD_NOD node, UCHAR * streams)
{
/**************************************
 *
...
static void compute_rse_streams(CompilerScratch* csb, RecordSelExpr* rse, UCHAR * streams)
{
/**************************************
 *
 ...
 **************************************/
	...
	for (ptr = rse->rse_relation, end = ptr + rse->rse_count; ptr < end; ptr++) {
		...
		if (node->nod_type != nod_rse) {
			...
			compute_rse_streams(csb, (RecordSelExpr*) node, streams);
		}
	}
}
...
/* turn a between into (a greater than or equal) AND (a less than  or equal) */
	...
	if (boolean_node->nod_type == nod_between) {
		...
		node = make_binary_node(tdbb, nod_geq, arg, boolean_node->nod_arg[1], true);
		LLS_PUSH(node, stack);
...
/* turn a LIKE into a LIKE and a STARTING WITH, if it starts
   with anything other than a pattern-matching character */
	...
	{
		node = make_binary_node(tdbb, nod_starts, boolean_node->nod_arg[0], arg,
							 false);
		LLS_PUSH(node, stack);
...
static USHORT distribute_equalities(thread_db* tdbb, LLS * org_stack, CompilerScratch* csb, USHORT base_count)
{
/**************************************
 *
 ...
 *
 **************************************/
	firebird::HalfStaticArray<LLS, OPT_STATIC_ITEMS> classes(tdbb->tdbb_default);
		...
		if (n >= 3)
			for (stack = *class_; stack; stack = stack->lls_next)
				for (temp = stack->lls_next; temp; temp = temp->lls_next) {
					boolean =
						make_binary_node(tdbb, nod_eql, (JRD_NOD) stack->lls_object,
			...
			if (search_stack(node1, *class_)) 
				...
				for (temp = *class_; temp; temp = temp->lls_next)
					if (!node_equality(node1, (JRD_NOD) temp->lls_object)) 
						 ...
						 * conjunct X(C,B)
						 */
						new_node = make_inference_node(tdbb, csb, boolean, arg1, arg2);
...
static bool dump_rsb(thread_db* tdbb, const jrd_req* request,
						RecordSource* rsb, UCHAR** buffer_ptr, SSHORT* buffer_length)
{
/**************************************
 *
 *	d u m p _ r s b
	...
	switch (rsb->rsb_type) 
		...
		case rsb_procedure:
			*buffer++ = isc_info_rsb_procedure;
			...
			if (!OPT_access_path(tdbb, procedure->findRequest(), buffer, *buffer_length, &return_length))
				return false;
 ...
 **************************************/
	...
	if (!done) {
		IndexedRelationship* relationship;
...
static USHORT find_order(thread_db* tdbb,
						 OptimizerBlk* opt, UCHAR * streams, JRD_NOD plan_node)
{
/**************************************
 *
 *	f i n d _ o r d e r
 ...
 **************************************/
	...
	if (opt_debug_flag >= DEBUG_BEST) {
		OptimizerBlk::opt_stream *order_end, *tail;
 ...
 **************************************/
	...
	switch (rsb->rsb_type) {
		...
		case rsb_ext_indexed:
			...
			LLS_PUSH((BLK)(long) rsb->rsb_stream, stream_list);
			return;
...
static void find_used_streams(RecordSource* rsb, UCHAR * streams)
{
/**************************************
 *
 ...
 **************************************/
	...
	OptimizerBlk::opt_stream *opt_end = opt->opt_streams.begin() + count;
	if (count != streams[0]) {
...
//			fb_assert(desc->dsc_dtype >= 0)
			...
			asb->nod_impure = CMP_impure(csb, sizeof(struct impure_agg_sort));
			asb->asb_desc = *desc;
...
static RecordSource* gen_boolean(thread_db* tdbb, OptimizerBlk* opt, RecordSource* prior_rsb, JRD_NOD node)
{
/**************************************
 *
...
static RecordSource* gen_first(thread_db* tdbb, OptimizerBlk* opt, RecordSource* prior_rsb, JRD_NOD node)
{
/**************************************
 *
 *	g e n _ f i r s t
 ...
 **************************************/
	...
	if (streams[0] == 1) {
		...
		if (csb_tail->csb_flags & csb_compute) {
			...
			fb_assert(relation);
			Format* format = CMP_format(tdbb, csb, streams[1]);
			fb_assert(format);
	...
	for (UCHAR* stream = streams + 1; stream < end_stream; stream++) {
		...
		Relation* relation = csb_tail->csb_relation;
		fb_assert(relation);
		Format* format = CMP_format(tdbb, csb, *stream);
...
static RecordSource* gen_navigation(thread_db* tdbb,
						  OptimizerBlk* opt,
						  USHORT stream,
						  Relation* relation, STR alias, index_desc* idx, JRD_NOD * sort_ptr)
{
/**************************************
 *
...
static RecordSource* gen_nav_rsb(thread_db* tdbb,
					   OptimizerBlk* opt,
					   USHORT stream, Relation* relation, STR alias, index_desc* idx
#ifdef SCROLLABLE_CURSORS
					   , RSE_GET_MODE mode
#endif
	)
 ...
 **************************************/
	...
	CompilerScratch* csb = opt->opt_csb;	
	Procedure * procedure = tdbb->tdbb_database->findProcedure(tdbb,(int)(long)node->nod_arg[e_prc_procedure]);
   ...
   inversion component of the boolean. */
		...
		)
	{
		...
		SSHORT idx_walk_count =
			sort_indices_by_priority(tdbb, csb_tail, idx_walk, idx_priority_level);
 ...
 **************************************/
	...
	if (rsb) {
		...
		if (opt->opt_g_flags & opt_g_stream) {
			size = sizeof(struct impure_inversion);
...
static RecordSource* gen_skip(thread_db* tdbb, OptimizerBlk* opt, RecordSource* prior_rsb, JRD_NOD node)
{
/**************************************
 *
 *	g e n _ s k i p
 ...
 **************************************/
    ...
    csb = opt->opt_csb;
    rsb = FB_NEW_RPT(*tdbb->tdbb_default, 0) RecordSource();   // was : rsb = (RecordSource*) ALLOCDV (type_rsb, 1);
    rsb->rsb_count = 1;
 ...
 **************************************/
	...
	SortMap* map;
	sort_key_def* sort_key;
	sort_key_def* end_key;
	DSC *desc, descriptor;
	Format* format;
   ...
   down. */
	...
	for (ptr = &streams[1]; ptr <= end_ptr; ptr++) {
		...
		while (SBM_next(csb->csb_rpt[*ptr].csb_fields, &id, RSE_get_forward)) {
			items++;
			LLS_PUSH((BLK) (long) id, &id_stack);
			LLS_PUSH((BLK)(long) * ptr, &stream_stack);
   ...
   it sufficiently large that there is room for a sort key descriptor on the
	...
	count = items +
		(sizeof(sort_key_def) * 2 * sort->nod_count + sizeof(smb_repeat) -
		 1) / sizeof(smb_repeat);
	map = FB_NEW_RPT(*tdbb->tdbb_default, count) SortMap();
	...
	sort_key = (sort_key_def*) & map->smb_rpt[items];
...
/* Now go back and process all to fields involved with the sort.  If the
   ...
   it. */
	while (stream_stack) {
		id = (USHORT)(long) LLS_POP(&id_stack);
		stream = (USHORT)(long) LLS_POP(&stream_stack);
 ...
 **************************************/
	...
	RecordSource* rsb;
	RecordSource* merge_rsb;
	RecordSource** rsb_tail;
	OptimizerBlk::opt_conjunct *tail, *end;
	DEV_BLKCHK(opt, type_opt);
	...
	for (stack1 = *org_rivers; stack1; stack1 = stack1->lls_next) {
		river1 = (River*) stack1->lls_object;
		...
		tail < end; tail++)
	{
		...
		for (stack1 = *org_rivers; stack1; stack1 = stack1->lls_next) {
			...
			for (stack2 = stack1->lls_next; stack2; stack2 = stack2->lls_next) {
				river2 = (River*) stack2->lls_object;
...
/* Pick both a set of classes and a set of rivers on which to join with
   ...
   to indicate that nothing could be done. */
	...
	// Build a sort stream.
	merge_rsb = FB_NEW_RPT(*tdbb->tdbb_default, river_cnt * 2) RecordSource();
	...
	river1 = FB_NEW_RPT(*tdbb->tdbb_default, stream_cnt) River();
	river1->riv_count = (UCHAR) stream_cnt;
	river1->riv_rsb = merge_rsb;
	stream = river1->riv_streams;
	stack1 = NULL;
	while (*org_rivers) {
		river2 = (River*) LLS_POP(org_rivers);
 ...
 **************************************/
	...
	csb = opt->opt_csb;
	rsb = FB_NEW_RPT(*tdbb->tdbb_default, count + nstreams + 1) RecordSource();
...
static void get_inactivities(CompilerScratch* csb, ULONG * dependencies)
{
/**************************************
 *
 ...
 **************************************/
	USHORT n;
	CompilerScratch::rpt_itr tail, end;
	DEV_BLKCHK(csb, type_csb);
...
static STR make_alias(thread_db* tdbb, CompilerScratch* csb, csb_repeat * base_tail)
{
/**************************************
 *
 *	m a k e _ a l i a s
	...
	for (csb_tail = base_tail;; csb_tail = &csb->csb_rpt[csb_tail->csb_view_stream]) 
		...
		if (csb_tail->csb_alias)
			...
			q = (!(csb_tail->csb_relation)
				 || !(csb_tail->csb_relation->rel_name)) ? NULL : (const char*) csb_tail->csb_relation->rel_name;
...
static JRD_NOD make_binary_node(thread_db* tdbb, NOD_T type, JRD_NOD arg1, JRD_NOD arg2, bool flag)
{
/**************************************
 *
 ...
 **************************************/
	...
	for (temp = stack, count = 0; temp; count++)
		temp = temp->lls_next;
	if (count == 1) {
		river = (River*) LLS_POP(&stack);
		return river->riv_rsb;
	}

	csb = opt->opt_csb;
	rsb = FB_NEW_RPT(*tdbb->tdbb_default, count) RecordSource();
	rsb->rsb_type = rsb_cross;
	rsb->rsb_count = count;
	rsb->rsb_impure = CMP_impure(csb, sizeof(struct irsb));
...
static JRD_NOD make_index_node(thread_db* tdbb, Relation* relation, CompilerScratch* csb, index_desc* idx)
{
/**************************************
 *
...
/* check whether this is during a compile or during
   a SET INDEX operation */
	...
	if (csb)
		node->nod_impure = CMP_impure(csb, sizeof(struct impure_inversion));
	return node;
}


static JRD_NOD make_inference_node(thread_db* tdbb, CompilerScratch* csb, JRD_NOD boolean, JRD_NOD arg1, JRD_NOD arg2)
{
/**************************************
 *
...
static JRD_NOD make_missing(thread_db* tdbb,
						OptimizerBlk* opt,
						Relation* relation, JRD_NOD boolean, USHORT stream, index_desc* idx)
{
/**************************************
 *
...
/* Every string starts with an empty string so
   don't bother using an index in that case. */
	...
	if (value->nod_type == nod_literal) {
		DSC *literal_desc;
		literal_desc = &((Literal*) value)->lit_desc;
	...
	for (i = 0; i < csb_tail->csb_indices; i++) {
		if (access_type) {
			for (arg = access_type->nod_arg, end = arg + plan_count;
				 arg < end; arg += 3) {
				if (relation_id != (SSHORT)(long) * arg) {
					/* index %s cannot be used in the specified plan */
					ERR_post(isc_index_unused, isc_arg_string, *(arg + 2), 0);
				}
				if (idx->idx_id == (USHORT)(long) * (arg + 1)) {
...
/* make room for an idx structure to describe the index
   that was used to generate this rsb */
	if (rsb->rsb_type == rsb_navigate)
		rsb->rsb_arg[RSB_NAV_idx_offset] = (RecordSource*) (long) size;
 ...
 *
 **************************************/
	OptimizerBlk::opt_stream *tail, *order_end;
	DEV_BLKCHK(opt, type_opt);
...
static bool river_reference(River* river, JRD_NOD node, bool *field_found)
{
/**************************************
 *
...
static void set_active(OptimizerBlk* opt, River* river)
{
/**************************************
 *
 *	s e t _ a c t i v e
...
static void set_inactive(OptimizerBlk* opt, River* river)
{
/**************************************
 *
...
static void set_made_river(OptimizerBlk* opt, River* river)
{
/**************************************
 *
...
static void set_rse_inactive(CompilerScratch* csb, RecordSelExpr* rse)
{
/***************************************************
 *
 ...
 ***************************************************/
		 ...
		 ptr < end; ptr++)
	{
		...
		if (node->nod_type != nod_rse) {
			...
			set_rse_inactive(csb, (RecordSelExpr*) node);
	}
}


static void sort_indices_by_selectivity(thread_db* tdbb, csb_repeat * csb_tail)
{
/***************************************************
 *
...
static SSHORT sort_indices_by_priority(thread_db* tdbb, csb_repeat * csb_tail,
									   index_desc** idx_walk,
