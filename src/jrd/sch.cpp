...
typedef struct thread {
	...
	AsyncEvent thread_stall[1];	/* Generic event to stall thread */
...
static USHORT multi_threaded = TRUE, enabled = FALSE;
 ...
 **************************************/
	...
	if (ast_thread && action == AST_check)
		if (!(ast_thread->thread_flags & THREAD_ast_pending) || ast_thread->thread_count > 1)
			return;

	if (!init_flag)
		init_flag = true; //SCH_init();
	...
	switch (action) 
		...
		case AST_exit:
			...
			for (thread = ast_thread->thread_next; thread != ast_thread;
				thread = thread->thread_next) ISC_event_post(thread->thread_stall);
			break;
		}
 ...
 **************************************/
	...
	if (thd = free_threads)
		free_threads = thd->thread_next;
	else 
		{
		thd = new thread; //(THREAD) gds__alloc((SLONG) sizeof(struct thread));

		if (!thd)			/* NOMEM: bugcheck */
		 ...
		 */
		gds_alloc_flag_unfreed((void *) thd);
#endif

		ISC_event_init(thd->thread_stall, 0, 0);
		}

	thd->thread_flags = thd->thread_count = 0;
...
/* loop through the list of active threads and free the events */
	if (temp_thread = active_thread) {
		 ...
		 * AsyncEvent* structures and move to the next */
		do {
