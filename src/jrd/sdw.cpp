...
static Shadow *allocate_shadow(DBB dbb, File *, USHORT, USHORT);
static BOOLEAN check_for_file(thread_db* tdbb, SCHAR *, USHORT);
#ifdef NOT_USED_OR_REPLACED
static void check_if_got_ast(File *);
#endif
static void copy_header(thread_db* tdbb);
static void update_dbb_to_sdw(Database *);


void SDW_add(thread_db* tdbb, TEXT * file_name, USHORT shadow_number, USHORT file_flags)
{
/**************************************
 *
 *	S D W _ a d d
 ...
 **************************************/
	...
	shadow_file = PIO_create(tdbb, file_name, strlen(file_name), FALSE, dbb->fileShared);

	if (dbb->dbb_flags & DBB_force_write)
		PIO_force_write(shadow_file, TRUE, dbb->fileShared);

	shadow = allocate_shadow(dbb, shadow_file, shadow_number, file_flags);
	...
	dbb->pageCache->writeAllShadows(tdbb, 0, window.win_bdb, 1, false);
...
int SDW_add_file(thread_db* tdbb, TEXT * file_name, SLONG start, USHORT shadow_number)
{
/**************************************
 *
...
// Verify shadow file path against DatabaseAccess entry of firebird.conf
	...
	if (!(sequence = PIO_add_file(tdbb, shadow_file, file_name, start)))
		return 0;
 ...
 **************************************/
	...
	for (shadow = dbb->dbb_shadow; shadow; shadow = next_shadow) 
		...
		if (shadow->sdw_flags & SDW_shutdown)
			SDW_shutdown_shadow(dbb, shadow);
		}
	...
	if (SDW_check_conditional(tdbb)) 
		...
		if (SDW_lck_update(dbb,  0)) 
			...
			lock->lck_owner_handle = LCK_get_owner_handle(tdbb, LCK_update_shadow);
	...
	for (page_number = HEADER_PAGE + 1; page_number <= max; page_number++) {
		...
		for (shadow = dbb->dbb_shadow; shadow; shadow = shadow->sdw_next)
			if (!(shadow->sdw_flags & (SDW_INVALID | SDW_dumped))) {
				...
				if (!dbb->pageCache->writeAllShadows(tdbb, shadow, window.win_bdb, 1, false))
					{
					CCH_RELEASE(tdbb, &window);
					ERR_punt();
					}
...
BOOLEAN SDW_lck_update(DBB dbb, SLONG sdw_update_flags)
{
/**************************************
 *
...
bool SDW_rollover_to_shadow(thread_db* tdbb, File *file, const bool inAst)
{
/**************************************
 *
 ...
 **************************************/
	...
	update_lock->lck_owner_handle = LCK_get_owner_handle(tdbb, LCK_update_shadow);
...
void SDW_shutdown_shadow(DBB dbb, Shadow *shadow)
{
/**************************************
 *
 ...
 **************************************/
	...
	UCHAR *p;
	File *dbb_file, *shadow_file = 0;
	header_page* database_header;
	header_page* shadow_header;
...
/* catch errors: delete the shadow file if missing,
   and deallocate the spare buffer */
	...
	try 
		{
		shadow_file = PIO_open(tdbb, expanded_name, FALSE, 0, file_name);
	...
	if (!(file_flags & FILE_conditional))
		...
		if (!PIO_read (shadow_file, window.win_bdb, (PAG) spare_page,
			 tdbb->tdbb_status_vector)) 
			ERR_punt();

		shadow_header = (header_page*) spare_page;
		...
		if (strncmp (dbb_file->fil_string, reinterpret_cast < const char *>(p),
						string_length)
						&& check_for_file(tdbb, reinterpret_cast < char *>(p),
						string_length)) 
			ERR_punt();
 ...
 **************************************/
	...
	new_dbb->pageCache->getShadows = true;
...
static Shadow *allocate_shadow(DBB dbb,
						   File *shadow_file,
...
static BOOLEAN check_for_file(thread_db* tdbb, SCHAR * name, USHORT length)
{
/**************************************
 *
 ...
 **************************************/
	...
	try 
		...
		File *temp_file = PIO_open(tdbb, name, FALSE, 0, name);
		PIO_close(temp_file);
		}
	catch (OSRIException&) 
		{
