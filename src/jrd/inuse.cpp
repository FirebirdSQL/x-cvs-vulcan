 ...
 **************************************/
	...
	do {
		...
		for (end = ptr + inuse->iuo_in_use_count; ptr < end; ptr++)
			if (*ptr) 
				{
				reinterpret_cast <void(*) (void*) >(*cleanup_routine) (*ptr);
	...
	if (secondary_inuse) 
		...
		Sync sync(&inuse_mutex, "INUSE_cleanup");
		sync.lock(Exclusive);
...
#endif
		Sync sync(&inuse_mutex, "INUSE_insert");
		sync.lock(Exclusive);
