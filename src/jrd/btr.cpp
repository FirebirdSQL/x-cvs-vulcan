...
static bool scan(thread_db*, UCHAR*, RecordBitmap**, index_desc*, 
				 ...
				 bool&, temporary_key&);
static void update_selectivity(Database* dbb, index_root_page*, USHORT, 
							   const SelectivityList&);
 ...
 **************************************/
	...
	idx->idx_root = fast_load(tdbb, relation, transaction, idx, key_length, 
...
bool BTR_description(thread_db* tdbb, Relation* relation, 
					 index_root_page* root, index_desc* idx, SSHORT id)
{
/**************************************
 *
	...
	if (retrieval->irb_upper_count)	
		...
		while (true) 
			...
			if (!node.isEndBucket) 
				...
				if (skipLowerKey)
					checkForLowerKeySkip(skipLowerKey, partLower, node, lower, 
						idx, retrieval);

				if (!skipLowerKey)
					RBM_SET(tdbb->tdbb_default, bitmap, node.recordNumber.getValue());
...
IDX_E BTR_key(thread_db* tdbb, Relation* relation, Record* record, index_desc* idx, 
...
USHORT BTR_key_length(thread_db* tdbb, Relation* relation, index_desc* idx)
{
/**************************************
 *
...
USHORT BTR_lookup(thread_db* tdbb, Relation* relation, USHORT id, index_desc* buffer)
{
/**************************************
 *
 *	B T R _ l o o k u p
 ...
 **************************************/
	...
	for (; id < root->irt_count; ++id) 
		...
		if (!irt_desc->irt_root && transaction
			...
			const int trans_state =
				TRA_wait(tdbb, transaction, trans, TRUE);
...
void BTR_selectivity(thread_db* tdbb, Relation* relation, USHORT id, SelectivityList& selectivity)
{
/**************************************
 *
	...
	while (page) 
		...
		while (true) 
			...
			if (segments > 1 && !firstNode) 
				...
				//Look for duplicates in the segments until *p1 != *p2
 ...
 **************************************/
	...
	if (itype == idx_string || itype == idx_byte_array ||
		...
		// clear key_empty flag, if length is >= 1 or
...
#ifdef DEBUG_INDEXKEY
		ib_fprintf(stderr, "NUMERIC %lg ", temp.temp_double);
#endif
		}
...
#ifdef DEBUG_INDEXKEY
		ib_fprintf(stderr, "TIMESTAMP2: %d:%u ",
				   ((const SLONG*) desc->dsc_address)[0],
				   ((const ULONG*) desc->dsc_address)[1]);
		ib_fprintf(stderr, "TIMESTAMP2: %20" QUADFORMAT "d ",
				   temp.temp_sint64);
#endif
		}
...
#ifdef DEBUG_INDEXKEY
		ib_fprintf(stderr, "DATE %d ", temp.temp_slong);
#endif
		}
...
#ifdef DEBUG_INDEXKEY
		ib_fprintf(stderr, "TIME %u ", temp.temp_ulong);
#endif
		}
...
#ifdef DEBUG_INDEXKEY
		ib_fprintf(stderr, "TIMESTAMP1 special %lg ", temp.temp_double);
#endif
		}
	else 
		{
...
#ifdef DEBUG_INDEXKEY
	{
		ib_fprintf(stderr, "temporary_key: length: %d Bytes: ", key->key_length);
		for (int i = 0; i < key->key_length; i++)
			ib_fprintf(stderr, "%02x ", key->key_data[i]);
		ib_fprintf(stderr, "\n");
	}
#endif
}
 ...
 **************************************/
	...
	try {
		...
		while (!error) 
			...
			if (segments > 1 && count > 1) 
				...
				if (newNode.prefix == 0) 
					...
					if (i == pos) 
						segment = *p2; // We _should_ pick number from data if available
					else 
						segment = *(p1 + i);
				...
				//Look for duplicates in the segment
		...
		jumpNodeListContainer::iterator itr;
		for (itr = jumpNodes->begin(); 	itr < jumpNodes->end(); ++itr) 
			{
		...
		keyList::iterator itr3;
		for (itr3 = jumpKeys->begin(); itr3 < jumpKeys->end(); ++itr3) 
			{
			delete[] (*itr3)->keyData;
			delete (*itr3);
			}
 ...
 **************************************/
	...
	// if this is an old style index the answer is easy.
	...
	// alas, it's not, so lets find a real starting point.
	...
	while (n) 
		...
		if ((jumpNode.prefix <= testPrefix) && descending) 
			...
			while (true) 
				...
				if (q == nodeEnd) 
					...
					if (useFindRecordNumber && (keyPointer == keyEnd)) 
						...
						while (n) 
							...
							if (node.length != 0 ||	node.isEndBucket || node.isEndLevel 
				...
				else if (retrieval && keyPointer == keyEnd) // time to stop?
					{
					done = true;
					break;
					}
				else if (keyPointer == keyEnd) 					
					break; // End of key reached
				else if (*keyPointer > *q) 
					break; // Our key is bigger so check next node.
				else if (*keyPointer++ < *q++) 
					{
					done = true;
					break;
			...
			while (true) 
				...
				if (keyPointer == keyEnd) 
					{
					done = true; // Reached end of our key we're searching for.
					...
					if (useFindRecordNumber && q == nodeEnd) 
						...
						while (n) 
							...
							// If the record number from leaf is higer
							if (find_record_number <= node.recordNumber) 
								break; // then we should be in our previous area.
					...
					break; // End of node data reached
				else if (*keyPointer > *q) 
					break; // Our key is bigger so check next node.
				else if (*keyPointer++ < *q++) 
					{
		...
		if (done) 
			break; // We're done, go out of main loop.
		else 
			{
 ...
 **************************************/
	...
	// previous page out before we write the tx_inv_page* page out
 ...
 **************************************/
	...
	if (useJumpInfo && splitJumpNodeIndex) 
		...
		if (endOfPage 
				&& ((splitpoint + jumpersNewSize - jumpersOriginalSize) <= 
...
static bool scan(thread_db* tdbb, UCHAR* pointer, RecordBitmap** bitmap,
				 ...
				 bool& skipLowerKey, temporary_key& lowerKey)
{
/**************************************
 *
 *	s c a n
	...
	if (page_flags & btr_large_keys) 
		...
		while (true) 
			...
			if ((key->key_length == 0) && !(key->key_flags & key_empty)) 
				{
				if (to_segment == 0)   // Scanning for NULL keys
					{
			...
			if (skipLowerKey)
				checkForLowerKeySkip(skipLowerKey, partLower, node, lowerKey, 
					*idx, retrieval);
		...
		while (true) 
			...
			if (skipLowerKey) 
				...
				if (node->btn_prefix == 0)
					...
					if (partLower) 
						...
						// With multi-segment compare first part of data with lowerKey
						skipLowerKey = 
							((lowerKey.key_length <= node->btn_length) &&
			...
			if (!ignore && !skipLowerKey) 
				{
				if ((flag & irb_starting) || !count)
					RBM_SET(tdbb->tdbb_default, bitmap, number);
				else if (p > (end_key - count))
					RBM_SET(tdbb->tdbb_default, bitmap, number);
				}
...
void update_selectivity(Database* dbb, index_root_page* root, 
						USHORT id, const SelectivityList& selectivity)
{
/**************************************
 *
