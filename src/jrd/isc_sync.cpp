...
#ifdef SOLARIS_MT
#define EVENTS
int ISC_event_blocked(USHORT count, AsyncEvent* * events, SLONG * values)
{
/**************************************
 *
...
SLONG ISC_event_clear(AsyncEvent* event)
{
/**************************************
 *
...
void ISC_event_fini(AsyncEvent* event)
{
/**************************************
 *
...
int ISC_event_init(AsyncEvent* event, int semid, int semnum)
{
/**************************************
 *
...
int ISC_event_post(AsyncEvent* event)
{
/**************************************
 *
...
int ISC_event_init(AsyncEvent* event, int type, int semnum)
{
/**************************************
 *
...
int ISC_event_init_shared(
	AsyncEvent* lcl_event,
	int type,
	const TEXT* name,
	AsyncEvent* shr_event,
	bool init_flag)
{
/**************************************
 *
 ...
 **************************************/
	...
	for (;;) 
		...
		const DWORD status = WaitForMultipleObjects((DWORD) count, handles, TRUE, timeout);
...
#ifdef WIN_NT
void *ISC_make_signal(BOOLEAN create_flag,
 ...
 **************************************/
	...
	TEXT event_name[64];
	sprintf(event_name, "_firebird_process%u_signal%d", process_id, signal_number);
 ...
 **************************************/
	...
	if (fcntl(fd, F_SETLK, &lock) != -1) 
		...
		if (shmem_data->sh_mem_semaphores &&
			 (semid = create_semaphores(status_vector, key, shmem_data->sh_mem_semaphores)) < 0) 
			{
#ifdef HAVE_FLOCK
			/* unlock both files */
			flock(fd, LOCK_UN);
...
#if !(defined SOLARIS_MT || defined POSIX_THREADS)
		if (shmem_data->sh_mem_semaphores &&
			(semid = open_semaphores(status_vector, key, shmem_data->sh_mem_semaphores)) < 0) {
#ifdef HAVE_FLOCK
			/* unlock both files */
			flock(fd, LOCK_UN);
			flock(fd_init, LOCK_UN);
...
/* If we're the only one with shared memory mapped, see if
   we can initialize it.  If we can't, return failure. */
	...
	if (shmem_data->sh_mem_semaphores &&
		(semid = (init_flag ? 
			create_semaphores(status_vector, key, shmem_data->sh_mem_semaphores) :
			open_semaphores(status_vector, key, shmem_data->sh_mem_semaphores)) ) < 0) 
		{
		shmdt(address);
		next_shared_memory -= length;
		ib_fclose(fp);
 ...
 **************************************/
	...
	for (int retry_count = 0;; ++retry_count)
		...
		if (!event_handle) 
			{
			error(status_vector, (ISC_is_WinNT()) ? "CreateEvent" : "CreateMutex", GetLastError());
...
static SLONG create_semaphores( SC_STATUS * status_vector, SLONG key, int semaphores)
{
/**************************************
 *
