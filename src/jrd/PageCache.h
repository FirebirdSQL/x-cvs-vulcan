...
public:
	...
	void initialize(tdbb *tdbb, int numberBuffers);
	bool writePage(tdbb *tdbb, Bdb* bdb, bool write_thru, bool inAst);
	void journalBuffer(Bdb* bdb);
	bool setWriteDirection(tdbb *tdbb, Bdb* bdb, int direction);
	bool rolloverToShadow(tdbb *tdbb, fil* file, bool inAst);
	bool writeAllShadows(tdbb *tdbb, sdw* shadow, Bdb* bdb, int checksum, bool inAst);
	void removeDirtyPage(Bdb* bdb);
	void unwind(tdbb* tdbb, bool punt);
	void releaseLatch(tdbb* tdbb, Bdb* bdb, bool repost, bool downgrade_latch, bool rel_mark_latch);
	void bugcheck(int msgNumber);
	void flushDatabase(tdbb* tdbb);
	void updateWriteDirection(tdbb* tdbb, Bdb* bdb);
	void invalidateAndRelease(tdbb* tdbb, Bdb* bdb);
	int downGradeDatabase(void);
	void insertDirtyPage(Bdb* bdb);
	pag* fake(tdbb* tdbb, win* window);
	Bdb* getBuffer(tdbb* tdbb, SLONG page, int lock_type);
	void clearPrecedence(Bdb* bdb);
	int writeBuffer(tdbb* tdbb, Bdb* bdb, SLONG page, bool write_thru, ISC_STATUS* status, bool write_this_page);
	LockState lockBuffer(tdbb* tdbb, Bdb* bdb, int wait, int page_type);
	void mark(tdbb* tdbb, win* window, int mark_system);
	int latchBdb(tdbb* tdbb, LatchType type, Bdb* bdb, SLONG page);
	bool writeable(Bdb* bdblock);
	void expandBuffers(tdbb* tdbb, int expandedSize);
	int blockingAst(Bdb* bdb);
	void downGrade(tdbb* tdbb, Bdb* bdb);
	bool getExclusive(tdbb* tdbb, int level, int wait_flag);
	bool getExclusiveAttachment(tdbb* tdbb, int level, int wait_flag);
	pag* fetch(tdbb* tdbb, win* window, int lock_type, int page_type, int checksum, int latch_wait, int read_shadow);
	LockState fetchLock(tdbb* tdbb, win* window, int lock_type, int wait, int page_type);
	void fetchPage(tdbb* tdbb, win* window, int compute_checksum, bool read_shadow);
	void pageValidationError(tdbb* tdbb, win* window, int type);
	pag* handoff(tdbb* tdbb, win* window, SLONG page, int lock, int page_type, int latch_wait, int release_tail);
	void unmark(tdbb* tdbb, win* window);
	void release(tdbb* tdbb, win* window, bool release_tail = false);
	void declarePrecedence(tdbb* tdbb, win* window, SLONG page);
	//void checkPrecedence(tdbb* tdbb, win* window, SLONG page);
	int related(Bdb* low, Bdb* high, int limit);
	void prefetch(tdbb* tdbb, SLONG* pages, int count);
	void markMustWrite(tdbb* tdbb, win* window);
	void flush(tdbb* tdbb, int flush_flag, int tra_number);
	void flushDirtyPages(tdbb* tdbb, int transaction_mask, bool sys_only, ISC_STATUS* status);
	void releaseExclusive(tdbb* tdbb);
	void shutdownDatabase(tdbb* tdbb);
	int checksum(Bdb *bdb);
	bool validate(win* window);
	void recoverShadow(tdbb* tdbb, sbm* sbm_rec);
	void fini(tdbb* tdbb);
