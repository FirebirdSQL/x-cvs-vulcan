...
int		ISC_event_blocked(USHORT, AsyncEvent **, SLONG *);
SLONG	ISC_event_clear(AsyncEvent *);
void	ISC_event_fini(AsyncEvent *);
int		ISC_event_init(AsyncEvent *, int, int);

#if defined(WIN_NT)
int		ISC_event_init_shared(AsyncEvent*, int, const TEXT*, AsyncEvent*, bool);
#endif
int		ISC_event_post(AsyncEvent *);
int		ISC_event_wait(SSHORT, AsyncEvent **, SLONG *, SLONG, FPTR_VOID_PTR, void *);
...
#ifdef SUPERSERVER
#ifdef UNIX
void	ISC_exception_post(ULONG, const char*);
void	ISC_sync_signals_set(void);
void	ISC_sync_signals_reset(void);
#endif /* UNIX */

#ifdef WIN_NT
ULONG	ISC_exception_post(ULONG, const char*);
#endif /* WIN_NT */
