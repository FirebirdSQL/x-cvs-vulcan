 ...
 **************************************/
	...
	/***
	NAMESPACE::vector<JrdMemoryPool*>::iterator itr;
...
#ifndef MEMMGR
	FOR_OBJECTS (JrdMemoryPool*, pool, &Dbb->dbb_pools)
		pool->verify_pool();
	END_FOR;
#endif
...
JrdMemoryPool *JrdMemoryPool::createPool(Database *dbb, int *cur_mem, int *max_mem) 
	{
#ifdef MEMMGR
	JrdMemoryPool *result = new JrdMemoryPool;
#else
...
JrdMemoryPool *JrdMemoryPool::createPool(Database *dbb) 
	{
#ifdef MEMMGR
	JrdMemoryPool *result = new JrdMemoryPool;
#else
...
{
	/***
	Database::PoolVector::iterator itr =
		std::find(dbb->dbb_pools.begin(), dbb->dbb_pools.end(), pool);
		
	if (itr != dbb->dbb_pools.end()) 
		dbb->dbb_pools.erase(itr);
	***/
	
	pool->database->removePool (pool);
 ...
 **************************************/
	...
	dbb->addPool (pool);
	dbb->dbb_bufferpool = JrdMemoryPool::createPool(tdbb->tdbb_database);
