...
static FPTR_SHORT lookup_init_function(thread_db* tdbb, USHORT, SSHORT, SSHORT);
 ...
 **************************************/
	...
	if (id >= dbb->dbb_charsets.count)
		dbb->dbb_charsets.extend(id + 10);
	else
		cs = (CharSetContainer*) dbb->dbb_charsets[id];

	// allocate a new character set object if we couldn't find one.

	if (!cs) 
		{
		cs = FB_NEW(*dbb->dbb_permanent) CharSetContainer(tdbb, dbb->dbb_permanent, id);
		if (cs->getCharSet() == NULL) 
			{
			delete cs;
			return NULL;
			}
		dbb->dbb_charsets.setElement (id, cs);
...
CharSetContainer::CharSetContainer(thread_db* tdbb, MemoryPool *p, USHORT cs_id) : cs(NULL)
{
	typedef USHORT (*CSInitFunc)(CHARSET, SSHORT, SSHORT);
	
	CSInitFunc csInitFunc = 
		reinterpret_cast<CSInitFunc>(lookup_init_function(tdbb, type_charset, cs_id, 0));
...
CharSetContainer::CharSetContainer(void) : cs(NULL)
{
	id = -1;
	cs = NULL;
}

CsConvert CharSetContainer::lookupConverter(thread_db* tdbb, int to_cs)
{
	int pos;
	...
	***/
	...
	if (impossible_conversions.findElement (to_cs))
		return NULL;
	...
	CVTInitFunc cvtInitFunc = 
		reinterpret_cast<CVTInitFunc>(lookup_init_function(tdbb, type_csconvert, to_cs, cs.getId()));

	if (!cvtInitFunc) 
		{
		impossible_conversions.append((void*) to_cs);
		return NULL;
		}
...
TextType CharSetContainer::lookupCollation(thread_db* tdbb, USHORT tt_id)
{
	...
	if (id < charset_collations.size() && (tt = charset_collations[id]))
		return charset_collations[id];
	
	typedef USHORT (*TTInitFunc)(TEXTTYPE, SSHORT, SSHORT);
	TTInitFunc ttInitFunc =
		reinterpret_cast<TTInitFunc>(lookup_init_function(tdbb, type_texttype, tt_id, 0));

	if (!ttInitFunc)
		return NULL;

	tt = FB_NEW(*tdbb->tdbb_database->dbb_permanent) texttype;
...
static FPTR_SHORT lookup_init_function(thread_db* tdbb, 
						USHORT type,
						SSHORT parm1,
						SSHORT parm2)
{
 ...
 *
 ***************************************/
	typedef USHORT(*LookupFn) (USHORT, FPTR_SHORT *, SSHORT, SSHORT);
	...
	if (function = INTL_builtin_lookup(type, parm1, parm2))
		return function;
	...
	Module *intlMod1 = database->intlModuleManager->findModule(INTL_MODULE1);

	if (intlMod1 && (lookup_fn = (LookupFn) intlMod1->findEntrypoint(tempStr)))
		if ((*lookup_fn)(type, &function, parm1, parm2) == 0)
			return function;
#endif
		//return function;
...
#else
	Module *intlMod2 = database->intlModuleManager->findModule(INTL_MODULE2);

	if (intlMod2 && (lookup_fn = (LookupFn) intlMod2->findEntrypoint(tempStr)))
		if ((*lookup_fn)(type, &function, parm1, parm2) == 0)
			return function;
	...
	if ( (function_block = FUN_lookup_function(tdbb, (TEXT*)entry, false)) ) 
		{
 ...
 **************************************/
	...
	if (cs_obj != NULL) 
		{
		len = cs_obj.convert(dest_ptr, dest_len, src_ptr, src_len, &err_code, &err_position);
		
		if (!err_code || 
			((err_code == CS_TRUNCATION_ERROR) &&  all_spaces(tdbb, src_type, src_ptr, src_len, err_position))) 
			return len;
	...
	if (from_cs == NULL)
		(*err)(isc_arith_except, isc_arg_gds, isc_text_subtype, isc_arg_number, src_type, 0);

	// allocate a temporary buffer that is large enough.
	
	TempSpace temp (src_len * sizeof(UCS2_CHAR));
	...
	if (to_cs == NULL) 
		(*err) (isc_arith_except, isc_arg_gds, isc_text_subtype, isc_arg_number, dest_type, 0);
		
	cs_obj = to_cs.getConvFromUnicode();
	fb_assert(cs_obj != NULL);
	len2 = cs_obj.convert(dest_ptr, dest_len, temp.space, len, &err_code, &err_position);
...
/* Should from_cs == to_cs? be handled better? YYY */
	...
	charset = dbb->findCharset (tdbb, from_cs);
...
int INTL_convert_string(const dsc* to, const dsc* from, FPTR_ERROR err)
{
/**************************************
 *
 ...
 **************************************/
	...
	CharSetContainer *cs = tdbb->tdbb_database->findCharset (tdbb, parm1);
 ...
 **************************************/
	...
	CharSetContainer *csc = tdbb->tdbb_database->findCharset (tdbb, parm1);
 ...
 **************************************/
	...
	USHORT len = CVT_get_string_ptr(pString, &ttype, &src, reinterpret_cast < vary * >(buffer),
...
bool CharSetContainer::isNamed(const char* name)
{
	return cs.isNamed (name);
}

bool CharSetContainer::loadCharSet(thread_db* tdbb, MemoryPool* p, int cs_id)
{
	typedef USHORT (*CSInitFunc)(CHARSET, SSHORT, SSHORT);
	CSInitFunc csInitFunc = 
		reinterpret_cast<CSInitFunc>(lookup_init_function(tdbb, type_charset, cs_id, 0));
...
int CharSetContainer::getBytesPerChar(void)
{
	return cs.maxBytesPerChar();
}

int CharSetContainer::getTType(void)
{
	//return INTL_CS_COLL_TO_TTYPE(0, id);
	return INTL_CS_COLL_TO_TTYPE(id, 0);
}

int CharSetContainer::getCharsetId(void)
{
	return id;
}

int CharSetContainer::getCollationId(void)
{
	return 0;
}
