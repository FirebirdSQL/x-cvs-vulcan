...
public:
	...
	class Element
		{
		public:
			Element (SIVector<Type> *vec, int idx)
				{
				parent = vec;
				index = idx;
				}
...
protected:
	...
	SIVector( const SIVector& );
	void operator=( const SIVector& );
	
public:
	// Allocate nothing by default
	SIVector() : buffer(0), length(0), true_length(0) {}

	// Allocate N elements
	SIVector(size_type n) : length(n), true_length(n) 
	{ 
		try 
			{ 
			buffer = new Type[n]; 
	...
	virtual ~SIVector() { delete[] buffer; }
	
	/*
	 * Access to an element is by the at() function or by operator[].
	 * Both return a reference (const or not, as appropriate).  
	 * To modify the contents of element i in SIVector<char> v: v[i] = 'a';
	 */
	// by function
	
	Type& at(size_type n) { return SIVector::buffer[n]; }
	const Type& at(size_type n) const { return SIVector::buffer[n]; }
	...
	// Change the capacity
	SIVector&  resize(size_type n, Type e = Type())
		{
		Sync sync (&syncObject, "SIVector::operator[]");
		sync.lock(Exclusive);
