...
static int			arg_is_string(SLONG, TEXT **, const TEXT *);
static SSHORT		compare_ASCII7z(const char *, const char *);
static SLONG		compile_module(SLONG,TEXT*);
static BOOLEAN		file_rename(TEXT *, const TEXT *, const TEXT *);
#ifdef GPRE_FORTRAN
...
//static FILE*		reposition_file(IB_FILE *, SLONG);
...
static int arg_is_string( SLONG argc, TEXT ** argvstring, const TEXT * errstring)
{
	TEXT *str;
...
//  Position the input file and initialize various modules 
	...
	traceFileName = TempFile::getTempFilename(SCRATCH);
	trace_file = TempFile::openTempFile (traceFileName);
...
static BOOLEAN file_rename(TEXT * file_name,
						   const TEXT * extension, 
						   const TEXT * new_extension)
{
	const  TEXT *q;
	TEXT *p, *terminator, *ext;
...
//  
	...
	comment_start_len = (int) strlen(comment_start);
	to_skip = 0;
...
//  We're out of actions -- dump the remaining text to the output stream. 
	...
	while ((c = get_char(input_file)) != EOF) {
		if (c == '\n' && line_pending) {
			fprintf(out_file, "\n#line %ld \"%s\"", line + 1, sanitized_file_name);
			line_pending = FALSE;
		}
		if (c == EOF) {
