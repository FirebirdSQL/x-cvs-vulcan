...
static void t_start_auto(ACT, GPRE_REQ, const TEXT *, int, SSHORT);

static int first_flag;
static const TEXT *status_name;
...
static SSHORT gen_event_block( ACT action)
{
	...
	init->nod_arg[2] = (GPRE_NOD) (long) ident;
...
static void gen_event_init( ACT action, int column)
{
	...
	args.pat_vector1 = status_vector(action);
	args.pat_value1 = (long) init->nod_arg[2];
	args.pat_value2 = (long) event_list->nod_count;
...
//  event has been initialized and getting the event identifier 
	...
	for (stack_ptr = events; stack_ptr; stack_ptr = stack_ptr->lls_next) {
		...
		if (!strcmp(event_name->sym_string, stack_name->sym_string)) {
			ident = (long) event_init->nod_arg[2];
...
static void gen_finish( ACT action, int column)
{
    ...
    isc_%S1_transaction (%V1, (isc_tr_handle*) &%S2);";
	...
	if (request->req_length) {
		...
		if (sw_raw) {
			...
			default:
				string_type = "blr";
				if (isc_print_blr
					((char *) request->req_blr, (FPTR_PRINT_CALLBACK) gen_blr, 0, 0))
...
static void make_ready( DBB db,
					   const TEXT * filename,
					   const TEXT * vector, 
					   USHORT column, GPRE_REQ request)
{
	TEXT s1[32], s2[32];
	const TEXT *dpb_size_ptr, *dpb_ptr;
...
static void t_start_auto(
						 ACT action,
						 GPRE_REQ request, const TEXT * vector, int column, SSHORT test)
{
	DBB db;
	int count, stat, remaining, length;
	const TEXT *filename, *trname;
	TEXT buffer[256];
