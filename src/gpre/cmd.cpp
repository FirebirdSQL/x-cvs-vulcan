...
static void create_constraint( GPRE_REQ request, ACT action, CNSTRT cnstrt)
{
	...
	for (; cnstrt; cnstrt = cnstrt->cnstrt_next) {
		...
		if (cnstrt->cnstrt_type == CNSTRT_FOREIGN_KEY) {
			...
			put_numeric(request, isc_dyn_idx_unique, TRUE);

		for (field = cnstrt->cnstrt_fields; field; field = field->lls_next) {
			string = (STR) field->lls_object;
			put_cstring(request, isc_dyn_fld_name, (TEXT *) string);
		}
		if (cnstrt->cnstrt_type == CNSTRT_FOREIGN_KEY) {
			...
			for (field = cnstrt->cnstrt_referred_fields; field;
				 field = field->lls_next) {
				string = (STR) field->lls_object;
				put_cstring(request, isc_dyn_idx_ref_column, (TEXT *) string);
			}
		}
		STUFF_END;
	}
...
//   put_cstring (request, isc_dyn_rel_constraint, cnstrt->cnstrt_name);
//***
...
static void create_index( GPRE_REQ request, IND index)
{
	...
	if (index->ind_flags & IND_descend)
		put_numeric(request, isc_dyn_idx_type, TRUE);
...
//  Name of trigger to be generated    
	...
	if (trigger->trg_message != NULL) {
		...
		put_cstring(request, isc_dyn_trg_msg, (TEXT *) trigger->trg_message);
		STUFF(isc_dyn_end);
	}
...
//  add the mapping from the rse to the view fields 
	...
	for (ptr = fields->nod_arg, end = ptr + fields->nod_count; ptr < end;
		...
		if (fld) {
			...
			put_blr(request, isc_dyn_fld_computed_blr, value,
					reinterpret_cast < pfn_local_trigger_cb > (CME_expr));
...
static void declare_udf( GPRE_REQ request, ACT action)
{
	...
	put_cstring(request, isc_dyn_func_entry_point, udf->decl_udf_entry_point);
	put_cstring(request, isc_dyn_func_module_name, udf->decl_udf_module_name);
...
static void put_short_cstring( GPRE_REQ request, USHORT operator_, const TEXT * string)
{
	int length = (string) ? (int) strlen (string) : 0;
	STUFF_CHECK(length);
...
static void put_string(GPRE_REQ request,
...
static void set_statistics( GPRE_REQ request, ACT action)
{
	...
	if (stats = (STS) action->act_object)
		if (stats->sts_flags & STS_index) {
			put_cstring(request, isc_dyn_mod_idx, (TEXT *) stats->sts_name);
