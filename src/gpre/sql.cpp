	...
	if ((where = MATCH(KW_WITH)) && MATCH(KW_CURRENT)) {
		...
		if ((transaction || request->req_trans) &&
			...
			if (transaction)
				...
				trans_nm_len = (int) strlen(request->req_trans);
...
static ACT act_update(void)
{
	...
	SLC slice;
	char		*transaction, r_name[NAME_SIZE + 1], db_name[NAME_SIZE + 1],
				owner_name[NAME_SIZE + 1], *str_2;
	const char	*str_1;
...
static char* par_options()
{
	...
	return PAR_native_value(FALSE, TRUE);
}
