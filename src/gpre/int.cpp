...
static void printa(int, const TEXT *, ...) ATTRIBUTE_FORMAT(2,3);
...
static void gen_request( GPRE_REQ request)
{
	...
	if (sw_raw)
		...
		gds__print_blr(request->req_blr, (FPTR_PRINT_CALLBACK) gen_blr, 0, 0);
...
static void gen_type( ACT action, int column)
{
	...
	printa(column, "%ld", (long) action->act_object);
}
