 ...
 */
	...
	if (KEYWORD(KW_SEMI) ||
		...
		if (MATCH(KW_ASTERISK))
			LEX_edit((SLONG) 0, (int)(long) statement_list->lls_object);
		else {
			if (KEYWORD(KW_SEMI))
				l = 1;
			...
			command_end();
			LEX_edit((SLONG) (long) start->lls_object, (SLONG) (long) stop->lls_object);
		}
	}
#ifdef PYXIS
 ...
 **************************************/
	...
	while (TRUE) {
		...
		if (op != nod_print_item) {
			...
			if ((op == nod_skip) && ((long) node->syn_arg[0] < 1))
