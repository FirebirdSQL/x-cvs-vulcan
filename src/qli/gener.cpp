...
#ifdef DEV_BUILD
static SCHAR explain_info[] = { isc_info_access_path };
#endif
 ...
 **************************************/
	...
	if (QLI_blr)
		isc_print_blr((const char*) rlb->rlb_base, 0, 0, 0);
 ...
 **************************************/
	...
	if ((NOD_T) (long) node->nod_arg[e_rse_join_type] == (NOD_T) 0)
		STUFF(blr_rse);
	else
		STUFF(blr_rs_stream);
...
/* Handle various clauses */
	...
	join_type = (NOD_T) (long) node->nod_arg[e_rse_join_type];
