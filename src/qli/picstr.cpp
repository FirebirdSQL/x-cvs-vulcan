...
static const TEXT *default_edit_string(DSC *, TEXT *);
...
PICS PIC_analyze(const  TEXT * string, DSC * desc)
{
/**************************************
 *
 ...
 **************************************/
	PICS picture;
	TEXT c, d, debit;
	const char *effectiveString = string;

	if (!effectiveString)
		{
		if (!desc)
			return NULL;
		effectiveString = default_edit_string(desc, NULL);
		}

	debit = 0;

	picture = (PICS) ALLOCD(type_pic);
	picture->pic_string = picture->pic_pointer = effectiveString;
...
static const TEXT *default_edit_string(
									DSC * desc, TEXT * buff)
{
/**************************************
 *
 ...
 **************************************/
	...
	if (buff == buffer) {
		string = (STR) ALLOCDV(type_str, (int) strlen(buff));
 ...
 **************************************/
	...
	DSC temp_desc;
	TEXT c, d, *p, *out, *year, *nmonth, *day,
