...
static BOOLEAN get_line(FILE *, TEXT *, USHORT);
 ...
 **************************************/
	...
	//scratch = (FILE *) isc_temp_file(TRUE, SCRATCH, filename);
	JString filename = TempFile::getTempFilename (SCRATCH);
	FILE *scratch = TempFile::openTempFile (filename, true);
 ...
 **************************************/
	...
	//trace_file = (FILE*) isc_temp_file(TRUE, SCRATCH, (TEXT*) trace_file_name);
	trace_file_name = TempFile::getTempFilename (SCRATCH);
	trace_file = TempFile::openTempFile (trace_file_name, TRUE);
 ...
 **************************************/
	...
	for (temp = QLI_line;
		 ...
		 temp = temp->line_next)
if (temp->line_next->line_position == (SLONG) (long) QLI_statements->lls_object) return;
...
void LEX_procedure( DBB database, isc_handle blob)
{
/**************************************
 *
...
int LEX_push_file(const TEXT * filename, int error_flag)
{
/**************************************
 *
 ...
 **************************************/
	...
	if (!(file = fopen(filename, FOPEN_INPUT_TYPE))) {
...
void LEX_put_procedure(isc_handle blob, SLONG start, SLONG stop)
{
/**************************************
 *
...
static BOOLEAN get_line( FILE * file, TEXT * buffer, USHORT size)
{
/**************************************
 *
 *	g e t _ l i n e
