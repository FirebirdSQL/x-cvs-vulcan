...
static void dump_procedure(DBB, IB_FILE *, TEXT *, USHORT, isc_handle );
 ...
 **************************************/
	...
	for (i = 0; i < node->syn_count; i++) {
		...
		switch (sw) {
		case set_blr:
			QLI_blr = (USHORT) (long) value;
			break;

		case set_statistics:
			QLI_statistics = (USHORT) (long) value;
			break;

		case set_columns:
			QLI_name_columns = QLI_columns = (USHORT) (long) value;
			break;

		case set_lines:
			QLI_lines = (USHORT) (long) value;
			break;

		case set_semi:
			QLI_semi = (USHORT) (long) value;
			break;

		case set_echo:
			QLI_echo = (USHORT) (long) value;
			break;
...
#ifdef DEV_BUILD
		case set_explain:
			QLI_explain = (USHORT) (long) value;
			break;

		case set_hex_output:
			QLI_hex_output = (USHORT) (long) value;
			break;
#endif
...
static void dump_procedure(
						   DBB database,
						   IB_FILE * file,
						   TEXT * name, USHORT length, isc_handle blob)
{
/**************************************
 *
