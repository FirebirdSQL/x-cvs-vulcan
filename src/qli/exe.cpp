...
isc_handle EXEC_open_blob( QLI_NOD node)
{
/**************************************
 *
 ...
 **************************************/
	...
	if (message) {
		...
		if (!isc_start_and_send(status_vector,
								 ...
								 (message->msg_buffer), 0)) return;
	}
	else
		if (!isc_start_request(status_vector,
								&request->req_handle,
								&request->req_database->dbb_transaction, 0)) return;
...
/* Format blob parameter block for the existing blob */
	...
	*p++ = to_desc->dsc_sub_type >> 8;
	...
	if (isc_create_blob(status_vector,
						 &to_dbb->dbb_handle,
						 &to_dbb->dbb_transaction,
						 &to_blob, (GDS__QUAD*) (to_desc->dsc_address)))
		ERRQ_database_error(to_dbb, status_vector);

	if (isc_open_blob2(status_vector,
						&from_dbb->dbb_handle,
						&from_dbb->dbb_transaction,
						&from_blob,
						(GDS__QUAD*) (from_desc->dsc_address),
						bpb_length,
 ...
 **************************************/
	...
	if (isc_request_info(status_vector,
						  &request->req_handle,
						  0,
						  sizeof(count_info),
						  count_info,
...
/* print out the counts of any records affected */
	...
	for (c = count_buffer; *c != isc_info_end; c += length) {
		item = *c++;
		length = isc_vax_integer(c, 2);
		c += 2;
		number = isc_vax_integer(c, length);
 ...
 **************************************/
	...
	if (database->dbb_transaction) {
		if (node->nod_type == nod_commit_retaining) {
			if (isc_commit_retaining(status,
									  &database->dbb_transaction))
					ERRQ_database_error(database, status);
		}
		else if (node->nod_type == nod_prepare) {
			if (isc_prepare_transaction(status,
										 &database->dbb_transaction))
					ERRQ_database_error(database, status);
		}
	}
