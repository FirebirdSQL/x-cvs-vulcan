 ...
 **************************************/
	...
	new_thread = new thread; //(THREAD) ALLR_alloc((SLONG) sizeof(struct thread));

	DuplicateHandle(GetCurrentProcess(), GetCurrentThread(),
					GetCurrentProcess(), &new_thread->thread_handle, 0, FALSE,
					DUPLICATE_SAME_ACCESS);

	Sync sync(&thread_mutex, "CNTL_insert_thread");
	sync.lock(Exclusive);
...
void WINAPI CNTL_main_thread( DWORD argc, LPTSTR* argv)
{
/**************************************
 *
 ...
 **************************************/
	...
	Sync sync(&thread_mutex, "CNTL_remove_thread");
	sync.lock(Exclusive);
 ...
 **************************************/
	...
	//JRD_num_attachments(return_buffer, ERROR_BUFFER_LENGTH, JRD_info_dbnames, &attach_count, &database_count);
	int attach_count = fb_shutdown_connections (fb_shutdown_count, 0);

	/* if we have active attachments then log messages */

	if (attach_count > 0) 
		{
		TEXT print_buffer[ERROR_BUFFER_LENGTH];

		sprintf(print_buffer, "Shutting down the Firebird service with %d active connection(s)", attach_count);
		...
		TEXT *print_ptr;
		USHORT database_name_length;
		int num_databases = (USHORT) * ((USHORT *) buff_ptr);
		...
		for (int i = 0; i < num_databases; i++) 
			...
			for (int j = 0; j < database_name_length; j++)
				*print_ptr++ = *buff_ptr++;
			*print_ptr = '\0';
			sprintf(out_message,
