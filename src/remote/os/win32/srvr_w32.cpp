...
static const char *configFile = "$(install)/server.conf";
static const char *configServer = "winserver";
	...
	***/
	...
	if (connection_handle != INVALID_HANDLE_VALUE) 
		...
		if (server_flag & SRVR_inet)
			...
			port = WNET_reconnect(configuration, connection_handle, status_vector);
#endif
#ifdef XNET
...
static int inet_connect_wait_thread( void* dummy)
{
/**************************************
 *
 ...
 **************************************/
	...
	while (true)
		{
		Port* port = WNET_connect(configuration, protocol_wnet, 0, status_vector, server_flag);
		if (!port) 
			{
 ...
 **************************************/
	...
	RServer server;
	server.processPackets(port, server_flag & ~SRVR_multi_client);
}
