	...
	switch (p->p_operation) 
		...
		case op_response_piggyback:
			...
			return xdr_status_vector(xdrs, response->p_resp_status_vector,
									reinterpret_cast <char**>(response->p_resp_strings)) ? P_TRUE : P_FALSE;
		...
		case op_event:
			...
			MAP(xdr_long, reinterpret_cast<SLONG&>(event->p_event_ast));	// no longer used
			MAP(xdr_long, event->p_event_arg);								// no longer used
 ...
 **************************************/
	...
	if (!cstring->cstr_address) 
		...
		if (!(cstring->cstr_address = new UCHAR [cstring->cstr_length]))
			return false;
 ...
 **************************************/
	...
	if (cstring->cstr_allocated) 
		{
		delete [] cstring->cstr_address; //ALLR_free(cstring->cstr_address);
 ...
 **************************************/
	...
	BLOB_PTR* p = buffer + (long) desc->dsc_address;
...
static bool_t xdr_message( XDR* xdrs, RMessage *message, FMT format)
{
/**************************************
 *
 ...
 **************************************/
	...
	switch (xdrs->x_op) 
		{
		case XDR_ENCODE:
			return (*xdrs->x_ops->x_putlong) (xdrs, reinterpret_cast <SLONG*>(&ip->bid_relation_id)) &&
				   (*xdrs->x_ops->x_putlong) (xdrs, reinterpret_cast <SLONG*>(&ip->bid_number));

		case XDR_DECODE:
			return (*xdrs->x_ops->x_getlong)(xdrs, reinterpret_cast <SLONG*>(&ip->bid_relation_id)) &&
				   (*xdrs->x_ops->x_getlong)(xdrs, reinterpret_cast <SLONG*>(&ip->bid_number));
 ...
 **************************************/
	...
	RRQ request = (RRQ) port->getObject (request_id);
	...
	rrq_repeat* tail = &request->rrq_rpt[message_number];
...
#ifdef VMS
static bool_t xdr_semi_opaque( XDR* xdrs, RMessage *message, FMT format)
{
/**************************************
 *
	...
	switch (xdrs->x_op) 
		...
		case XDR_DECODE:
			...
			if (!slice->lstr_address) 
				{
				if (!(slice->lstr_address = new UCHAR [slice->lstr_length]))
 ...
 **************************************/
	...
	if (statement_id >= 0) 
		{
		if (!(statement = (RSR) port->getObject(statement_id)))
			return FALSE;
		}
	else 
		statement = port->getStatement();
	...
	RFmt* fmt_ptr = (direction) ? &statement->rsr_select_format : 
								  &statement->rsr_bind_format;
	...
	if (xdrs->x_op == XDR_DECODE) 
		...
		if (blr->cstr_length) 
			{
			RMessage *temp_msg = PARSE_messages(blr->cstr_address, blr->cstr_length);
			if (temp_msg != (REM_MSG) -1) 
				{
				*fmt_ptr = temp_msg->msg_format;
				delete temp_msg;
				}
			}
		}
 ...
 **************************************/
	...
	if (!procedure)
		procedure = port->port_rpr = new RProcedure (port->port_context);

	procedure->clear();
	
	/* Parse the blr describing the message. */

	if ((message = PARSE_messages(blr->cstr_address, blr->cstr_length)) != (REM_MSG) -1) 
		{
		while (message) 
			{
...
*/
	...
	statement = (RSR) port->getObject(statement_id);
...
void Packet::zap(bool newPacket)
{
	if (newPacket)
		...
		memset(&p_operation, 0, sizeof(Packet) - OFFSET(Packet*, p_operation));
#else
		memset(this, 0, sizeof(Packet));
#endif
}
