...
static PortInet*	alloc_port(ConfObject *configuration, PORT);
static PortInet*	aux_connect(PORT, PACKET *, XDR_INT(*)(void));
static PortInet*	aux_request(PORT, PACKET *);
...
static PORT		inet_try_connect(ConfObject *configuration,
								 PACKET*,
									RDB,
									USHORT,
									TEXT*,
...
/* Try connection using first set of protocols.  punt if error */
	...
	PORT port = inet_try_connect(configuration, packet, rdb, *file_length, file_name,
 ...
 **************************************/
	...
	PortInet *port = alloc_port(configuration, 0);
	...
	if (packet) 
		...
		if (host_addr.s_addr == INADDR_NONE) 
			...
			sprintf(msg,
					...
					H_ERRNO);
			gds__log(msg, 0);
			port->genError(isc_network_error,isc_arg_string,port->port_connection->str_data,
...
static PortInet* alloc_port(ConfObject *configuration, PORT parent)
{
/**************************************
 *
 *	a l l o c _ p o r t
 ...
 **************************************/
	...
	if (first_time == true)
		...
		INET_remote_buffer = configuration->getValue(TcpRemoteBufferSize,TcpRemoteBufferSizeValue);
...
/* If this is a server, we're got an auxiliary connection.  Accept it */
	...
	PortInet *new_port = alloc_port(port->configuration, port->port_parent);
...
/* Set up new socket */
	...
	address.sin_family = AF_INET;
	in_addr bind_addr = get_bind_address(port->configuration);
	inet_copy(reinterpret_cast<const SCHAR*>(&bind_addr),
				(SCHAR*) &address.sin_addr,
				sizeof(address.sin_addr));
				
	//address.sin_port = htons(Config::getRemoteAuxPort());
	address.sin_port = port->configuration->getValue (RemoteAuxPort,RemoteAuxPortValue);
 ...
 **************************************/
	...
	PortInet *port = alloc_port(main_port->configuration, main_port);
	socklen_t l = sizeof(address);
...
static PORT inet_try_connect(ConfObject *configuration,
							 ...
							 const UCHAR* dpb, SSHORT dpb_length)
{
/**************************************
 *
...
/* If we can't talk to a server, punt.  Let somebody else generate
   an error.  status_vector will have the network error info. */
	...
	PORT port = INET_connect(node_name, configuration, packet, status_vector, FALSE, dpb, dpb_length);
