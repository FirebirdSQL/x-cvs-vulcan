 ...
 **************************************/
	...
	for (RSR statement = transaction->rtr_rdb->rdb_sql_requests; statement; statement = statement->rsr_next)
		{
...
/* See if we already know about the request level */
	...
	const rrq_repeat* const end = tail + request->rrq_max_msg;
	...
	for (; tail <= end; tail++) 
		...
		if (!format)
			continue;
		REM_MSG msg = new RMessage (format);
		tail->rrq_xdr = msg;
		msg->msg_next = msg;
...
//					break;
	...
	if (!got_dpb_connect_timeout || !got_dpb_dummy_packet_interval) 
		...
		if (!got_dpb_connect_timeout)
			...
			port->port_connect_timeout = port->configuration->getValue(ConnectionTimeout,ConnectionTimeoutValue);

		if (!got_dpb_dummy_packet_interval) 
			{
			port->port_flags |= PORT_dummy_pckt_set;
			//port->port_dummy_packet_interval = Config::getDummyPacketInterval();
			port->port_dummy_packet_interval = port->configuration->getValue(DummyPacketInterval,DummyPacketIntervalValue);
			}
		}
 ...
 **************************************/
	...
	for (;;) 
		{
		rrq_repeat* tail = request->rrq_rpt;
		rrq_repeat* const end = tail + request->rrq_max_msg;
		for (; tail <= end; tail++)
			{
 ...
 **************************************/
	rrq_repeat* tail = request->rrq_rpt;
	rrq_repeat* end = tail + request->rrq_max_msg;
	for (; tail <= end; tail++) {
	    REM_MSG message = tail->rrq_message;
 ...
 **************************************/
	...
	if (!attach_failures)
		{
		attach_failures = new TEXT [ATTACH_FAILURE_SPACE];
