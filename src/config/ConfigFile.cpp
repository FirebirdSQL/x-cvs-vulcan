...
InputFile* ConfigFile::openConfigFile(void)
{
	...
	const char *fileName = getenv ("VULCAN_CONF");
	
	if (fileName && inputFile->openInputFile (expand(fileName)))
		return inputFile;
	
	if (inputFile->openInputFile ("vulcan.conf"))
		return inputFile;
	
	if (inputFile->openInputFile ("~/.vulcan.conf"))
		return inputFile;
	
	JString client = getInstallDirectory();
	client += "/client.conf";
	
	if (inputFile->openInputFile (client))
		return inputFile;
...
{
	Element *element = findGlobalAttribute ("RootDirectory");
	
	if (element)
		{
		Element *value = element->findAttribute (0);
		if (value)
			return value->name;
		}
