...
static const char *string1 = "hi, there";
...
{
	return (int)(long) sysConfig.values[KEY_SORT_MEM_BLOCK_SIZE];
}

int Config::getSortMemUpperLimit()
{
	return (int)(long) sysConfig.values[KEY_SORT_MEM_UPPER_LIMIT];
}

bool Config::getRemoteFileOpenAbility()
{
	return (bool) (long) sysConfig.values[KEY_REMOTE_FILE_OPEN_ABILITY];
}

int Config::getGuardianOption()
{
	return (int)(long) sysConfig.values[KEY_GUARDIAN_OPTION];
}

int Config::getCpuAffinityMask()
{
	return (int)(long) sysConfig.values[KEY_CPU_AFFINITY_MASK];
}
...
{
	return (int)(long) sysConfig.values[KEY_TCP_REMOTE_BUFFER_SIZE];
}
...
{
	return (int)(long) sysConfig.values[KEY_IPC_MAP_SIZE];
}

int Config::getDefaultDbCachePages()
{
	return (int)(long) sysConfig.values[KEY_DEFAULT_DB_CACHE_PAGES];
}

int Config::getConnectionTimeout()
{
	return (int)(long) sysConfig.values[KEY_CONNECTION_TIMEOUT];
}

int Config::getDummyPacketInterval()
{
	return (int)(long) sysConfig.values[KEY_DUMMY_PACKET_INTERVAL];
}

int Config::getLockMemSize()
{
	return (int)(long) sysConfig.values[KEY_LOCK_MEM_SIZE];
}

int Config::getLockSemCount()
{
	return (int)(long) sysConfig.values[KEY_LOCK_SEM_COUNT];
}

int Config::getLockSignal()
{
	return (int)(long) sysConfig.values[KEY_LOCK_SIGNAL];
}
...
{
	return (int)(long) sysConfig.values[KEY_LOCK_HASH_SLOTS];
}

int Config::getLockAcquireSpins()
{
	return (int)(long) sysConfig.values[KEY_LOCK_ACQUIRE_SPINS];
}

int Config::getEventMemSize()
{
	return (int)(long) sysConfig.values[KEY_EVENT_MEM_SIZE];
}

int Config::getDeadlockTimeout()
{
	return (int)(long) sysConfig.values[KEY_DEADLOCK_TIMEOUT];
}

int Config::getSolarisStallValue()
{
	return (int)(long) sysConfig.values[KEY_SOLARIS_STALL_VALUE];
}
...
{
	int rc = (int)(long) sysConfig.values[KEY_PRIORITY_SWITCH_DELAY];
	if (rc < 1)
		rc = 1;
	return rc;
}
...
{
	int rc = (int)(long) sysConfig.values[KEY_DEAD_THREADS_COLLECTION];
	if (rc < 1)
		rc = 1;
	return rc;
}
...
{
	int rc = (int)(long) sysConfig.values[KEY_PRIORITY_BOOST];
	if (rc < 1)
		rc = 1;
	if (rc > 1000)
		rc = 1000;
...
{
	return (int)(long) sysConfig.values[KEY_REMOTE_SERVICE_PORT];
}
...
{
	return (int)(long) sysConfig.values[KEY_MAX_UNFLUSHED_WRITES];
}

int Config::getMaxUnflushedWriteTime()
{
	return (int)(long) sysConfig.values[KEY_MAX_UNFLUSHED_WRITE_TIME];
}

int Config::getProcessPriorityLevel()
{
	return (int)(long) sysConfig.values[KEY_PROCESS_PRIORITY_LEVEL];
}
...
{
#ifdef SUPERSERVER
	return (int)(long) sysConfig.values[KEY_REMOTE_AUX_PORT];
#else
	return 0;
#endif
}
...
{
	return (int)(long) sysConfig.values[KEY_TRACE_DSQL];
}
