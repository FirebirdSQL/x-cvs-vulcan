 ...
 **/
void GEN_expr( CStatement* request, dsql_nod* node)
{
	UCHAR blr_operator;
	dsql_nod* ddl_node;
	dsql_ctx* context;
 ...
 **/
void GEN_port( CStatement* request, dsql_msg* message)
{
	...
	dsql_str* buffer = FB_NEW_RPT(*request->threadData->tsql_default, new_len) dsql_str;
	...
	for (parameter = message->msg_parameters; parameter;
		...
		message->msg_buffer + (long) parameter->par_desc.dsc_address;
		}
	***/
}
 ...
 **/
void GEN_request( CStatement* request, dsql_nod* node)
{
	if (request->req_type == REQ_DDL) 
		{
	...
	if (request->flags & REQ_blr_version4)
 ...
 **/
void GEN_statement( CStatement* request, dsql_nod* node)
{
	dsql_nod* temp;
	dsql_nod** ptr;
	...
	case nod_exec_procedure:
		if (request->req_type == REQ_EXEC_PROCEDURE) {
			if (message = request->receiveMessage) {
				stuff(request, blr_begin);
 ...
 **/
static void gen_constant( CStatement* request, dsc* desc, bool negate_value)
{
	SLONG value;
	SINT64 i64value;
...
static void gen_error_condition( CStatement* request, const dsql_nod* node)
{
	...
	case nod_sqlcode:
		...
		stuff_word(request, (USHORT)(long) node->nod_arg[0]);
		return;
 ...
 **/
static void gen_for_select( CStatement* request, dsql_nod* for_select)
{
	...
	if (!for_select->nod_arg[e_flp_action] && !(request->dbb_flags & DBB_v3))
 ...
 **/
static void gen_map( CStatement* request, dsql_map* map)
{
	USHORT count = 0;
	dsql_map* temp;
 ...
 **/
static void gen_relation( CStatement* request, dsql_ctx* context)
{
	...
	if (relation) 
		...
		if (DDL_ids(request)) 
			...
			if (relation->rel_id < 0)
				ERRD_post(isc_sqlerr, isc_arg_number, (SLONG) - 204, 
							...
							isc_arg_gds, isc_random, isc_arg_string, (const char*) relation->rel_name, 0); 
							//isc_arg_gds, isc_random, isc_arg_string, linecol, 0);
		...
		if (DDL_ids(request)) 
			...
			stuff_word(request, procedure->findId());
			}
		else 
		***/
			{
			stuff(request, blr_procedure);
			stuff_cstring(request, procedure->findName());
			}
 ...
 **/
static void gen_select( CStatement* request, dsql_nod* rse)
{
	dsql_fld* field;
	dsql_rel* relation;
	dsql_ctx* context;
	...
	SSHORT constant;
	dsc constant_desc (dtype_short, sizeof(SSHORT), &constant);
	...
	par* parameter_eof = request->makeParameter (request->receiveMessage, false, false, 0);
		...
		for (const dsql_nod* const* const end2 = ptr + list->nod_count;  ptr < end2; ptr++) 
			...
			if (item && item->nod_type == nod_relation) 
				...
				if (relation = context->ctx_relation) 
					...
					// Set up dbkey
					par* parameter =
						request->makeParameter (request->receiveMessage, false, false, 0);
					...
					if (!(request->dbb_flags & DBB_v3)) 
						{
						parameter = request->makeParameter (request->receiveMessage, false, false, 0);
...
/* define the parameters for the scrolling message--offset and direction, 
   in that order to make it easier to generate the request */
	...
	{
		par* parameter = request->makeParameter (request->req_async, false, false, 0);
		...
		parameter = request->makeParameter (request->req_async, false, false, 0);
	...
	if ((message = request->sendMessage) != NULL) 
		{
		stuff(request, blr_receive);
		stuff(request, message->msg_number);
		}
	...
	for (par* parameter = message->msg_parameters; parameter; parameter = parameter->par_next) 
		{
 ...
 **/
static void gen_sort( CStatement* request, dsql_nod* list)
{
 ...
 **/
static void stuff_cstring( CStatement* request, const char* string)
{
	UCHAR c;
 ...
 **/
static void stuff_word( CStatement* request, USHORT word)
{
