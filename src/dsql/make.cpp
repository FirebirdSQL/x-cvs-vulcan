...
dsql_nod* MAKE_constant(thread_db* threadData, dsql_str* constant, dsql_constant_type numeric_flag)
{
	...
	dsql_nod* node = FB_NEW_RPT(*threadData->tsql_default,
	...
	switch (numeric_flag)
		...
		case CONSTANT_SLONG:
			...
			UCHAR *p = (UCHAR*) node->nod_arg; 
			node->nod_desc.dsc_address = p;
			*(SLONG*) p = (SLONG) (long) constant;
			}
			break;
		...
		case CONSTANT_SINT64:
			{
				...
				// valid - they start with X and constain only 0-9, A-F.
				...
				if (*p == 'X')
					...
					*p++; // skip the 'X' part.
					...
					// if value is negative, then gen_constant (from dsql/gen)
...
dsql_nod* MAKE_field(thread_db* threadData, dsql_ctx* context, dsql_fld* field, dsql_nod* indices)
{
	...
	if (field->fld_dimensions) 
		...
		if (indices) 
			...
			MAKE_desc_from_field(threadData, &node->nod_desc, field);
...
dsql_nod* MAKE_list(thread_db* threadData, Stack *stack)
{
	...
	USHORT count = stack->count();

	dsql_nod* node = MAKE_node(threadData, nod_list, count);
 ...
 **/
#ifdef UNDEF
par* MAKE_parameter(CStatement* request, dsql_msg* message, bool sqlda_flag, bool null_flag,
	USHORT sqlda_index)
{
...
// If a null handing has been requested, set up a null flag 
	...
	if (null_flag) {
		par* null = MAKE_parameter(request, message, false, false, 0);
				...
				const TEXT* name, USHORT length, SYM_TYPE type, dsql_req* object)
{
	...
	dsql_sym* symbol = FB_NEW_RPT(*threadData->tsql_default, length) dsql_sym;
...
dsql_str* MAKE_tagged_string(thread_db* threadData, const char* str_, size_t length, const char* charset)
{
	...
	dsql_str* string = FB_NEW_RPT(*threadData->tsql_default, length) dsql_str;
...
dsql_nod* MAKE_trigger_type(thread_db* threadData, dsql_nod* prefix_node, dsql_nod* suffix_node)
{
	...
	int suffix = mover.getLong (&suffix_node->nod_desc, 0);
	delete prefix_node;
	delete suffix_node;
	return MAKE_constant(threadData, (dsql_str*) (prefix + suffix - 1), CONSTANT_SLONG);
}
...
{
	var* variable = FB_NEW_RPT(*threadData->tsql_default, strlen(name)) var;
	dsql_nod* node = MAKE_node(threadData, nod_variable, e_var_count);
