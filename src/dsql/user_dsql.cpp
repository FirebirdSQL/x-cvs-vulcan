...
struct stmt
{
	...
	isc_handle	stmt_handle;		// stmt handle returned by dsql_xxx 
	isc_handle	stmt_db_handle;		// database handle for this statement 
};
...
//static void		cleanup_database(isc_handle*, SLONG);
...
/* If a new statement was allocated, add it to the symbol table and insert it
   into the list of statements */
	...
	if (!statement)
	{
		...
		statement->stmt_db_handle = (isc_handle) * db_handle;
		statement->stmt_stmt =
...
#pragma FB_COMPILER_MESSAGE("reinterpret_cast<> changes level of indirection.")
	const ISC_STATUS s = isc_dsql_fetch(	user_status,
						reinterpret_cast<isc_handle*>(stmt_handle),
						dialect,
...
ISC_STATUS API_ROUTINE isc_dsql_finish(isc_handle* db_handle)
{
	return 0;
}

ISC_STATUS API_ROUTINE isc_execute(ISC_STATUS* status_vector,
							   isc_handle* tra_handle,
							   const SCHAR* statement_name, SQLDA* sqlda)
{
...
ISC_STATUS API_ROUTINE gds__dsql_finish(isc_handle* db_handle)
{
...
//
#ifdef UNDEF // get rid of crap
static void cleanup_database(isc_handle* db_handle, SLONG dummy)
{
...
//
static void init(isc_handle* db_handle)
{
