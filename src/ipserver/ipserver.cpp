...
static ITR make_transaction(IDB, isc_handle );
 ...
 **************************************/
	...
	SLONG slice_length, return_length, *params, parameters[32];
	CHAR *sdl, sdl_buf[256], *slice;
	struct bid array_id;
	ips_slice *ips;
	ips_string *ips_sdl;
	...
	if (sdl_length <= (SLONG) sizeof(sdl_buf))
		...
		sdl = (char*) get_buffer(comm, sdl_length, IPS_SLICE_SDL);
		
	NOT_NULL(sdl, sdl_length);
	IPS_SERVER(comm, IPS_SLICE_SDL, (UCHAR*) sdl, sdl_length);
	...
	if (param_length <= (SLONG) sizeof(parameters))
		...
		params = (SLONG *) get_buffer(comm, param_length, IPS_SLICE_PARAM);
	...
	if (slice_length)
		{
		slice = (char*)gds__alloc(slice_length);
		NOT_NULL(slice, TRUE);
	...
	IPS_SERVER(comm, IPS_SLICE_BUFFER, (UCHAR*) slice, slice_length);
 ...
 **************************************/
	...
	ips = &comm->ips_operations.ips_op_object;
	handle = (isc_handle )ips->ips_handle;
...
static ITR make_transaction( IDB idb, isc_handle  handle)
{
/**************************************
 *
 ...
 **************************************/
	...
	events = (char*) get_buffer(comm, length, IPS_QUEUE_EVENT);
	NOT_NULL(events, length);
	IPS_SERVER(comm, IPS_QUEUE_EVENT, (UCHAR*) events, length);
 ...
 **************************************/
	...
	if (!GDS_RECONNECT(status_vector,
		...
		if (transaction = make_transaction(idb, handle))
			...
			{
			gds__handle_cleanup(status_vector, (isc_handle *) &handle);
			NOT_NULL(transaction, TRUE);
			}
		}
 ...
 **************************************/
	...
	GDS_SEEK_BLOB(status_vector,
				  &blob->ibl_handle, mode, offset, (&result));
			...
			while (idb->idb_transactions)
			{
				if (!(idb->idb_transactions->itr_flags & ITR_limbo))
					GDS_ROLLBACK(status_vector,
								 (&idb->idb_transactions->itr_handle));
				else
