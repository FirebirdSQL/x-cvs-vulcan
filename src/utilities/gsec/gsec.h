...
#endif
#ifdef SUPERSERVER
#define GET_THREAD_DATA	        ((TSEC) THD_get_specific(THDD_TYPE_TSEC))
#define SET_THREAD_DATA         THD_put_specific ((THDD) tdsec, THDD_TYPE_TSEC); \
				tdsec->tsec_thd_data.thdd_type = THDD_TYPE_TSEC
#define RESTORE_THREAD_DATA     THD_restore_specific(THDD_TYPE_TSEC);
#else
