	...
	*p = '\0';
	...
	switch (sw_command)
	{
		case COMMAND_INSTALL:
			...
			if (ret != FB_SUCCESS)
				printf ("Firebird Vulcan has not been installed in the registry.\n");
			else
				printf("Firebird Vulcan has been successfully installed in the registry.\n");
			break;

		case COMMAND_REMOVE:
			ret = REGISTRY_remove(hkey_node, false, reg_error);
			if (ret != FB_SUCCESS)
				printf("Firebird Vulcan has not been deleted from the registry.\n");
			else
				printf("Firebird Vulcan has been successfully deleted from the registry.\n");
			break;
 ...
 **************************************/
	...
	printf("  of your Firebird Vulcan installation.\n\n");
