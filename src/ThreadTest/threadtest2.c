...
// Do select * from table where intcol > row
            sprintf(sql, "select INTCOL, DBLCOL, CHARCOL, VCHARCOL from x%d where INTCOL >= ?", args->tableID);
        ...
        {
            // Only one param ...
            if (paramsqlda->sqld != 1)
                fberror(args, "expected 1 parameter");

            if (paramsqlda->sqlvar->sqllen != sizeof(row))
                fberror(args, "expected an int parameter");

            // Initialize the ind ...
            *(paramsqlda->sqlvar->sqlind) = 0;
        ...
        for (row = 0; row < NUM_LOOPS; row++)
        {
            ...
            if (paramsqlda && paramsqlda->sqld)
                *(int*)(paramsqlda->sqlvar->sqldata) = row * (NUM_ROWS / NUM_LOOPS + 1);
