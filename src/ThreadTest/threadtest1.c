...
// If set to 0 each thread will use it's own connection
// If set to 1 all threads will use a common connection
// If set to 2 half of the threads will use a common connection
// If set to 3 one third of the threads will use a common connection
// etc.
#define COMMON_CONNECT  3
...
//#else
//#define fbLock                  pthread_mutex_t
//#define fbInitLock(plock)       (plock ? pthread_mutex_init(plock, NULL) : 0)
//#define fbDestroyLock(plock)	  (plock ? pthread_mutex_destroy(plock) : 0)
//#define fbGetLock(plock)        (plock ? pthread_mutex_lock(plock) : 0)
//#define fbReleaseLock(plock)    (plock ? pthread_mutex_unlock(plock) : 0)
//#endif
//
//#else
...
int ThreadFn(Threadargs args)
{
   ...
   {
       // Get new db handle
       if (fbconnect(args, args->database, &(args->db)))
           fberror(args, "threadfn connect");
       threadConnect = 1;
   }
   ...
   if (args->doCreates)
   {
        ...
        if (fbexeci(args, &(args->db), &tr1, sql))
   ...
   if (args->doFetch)
   {
        ...
        if (fballocStmt(args, &(args->db), &stmt1))
...
//            sprintf(sql, "select * from x%d", args->tableID);
        ...
        if (fbfreeStmt(args, &stmt1, DSQL_drop))
            fberror(args, "drop statement");

   } // if we should fetch the rows

   // Disconnect if we had a connection just for this thread
   if (threadConnect)
   {
       if (fbdisconnect(args, &(args->db)))
           fberror(args, "threadfn disconnect");
   }
...
int main(int argc, char* argv[])
{
   ...
   if (COMMON_CONNECT)
   {
       if (fbconnect(args, argv[1], &db))
           fberror(args, "common connect");
   }
...
again:
   ...
   {   
       // Pass in the db handle to use ...
       if (COMMON_CONNECT &&
           ((thread % COMMON_CONNECT) == 0))
           args[thread].db = db;
       ...
       // isn't sharing one with the other threads
...
//          Sleep(1);
   ...
   // Disconnect if we have a common db handle
   if (COMMON_CONNECT)
   {
       if (fbdisconnect(args, &db))
           fberror(args, "common disconnect");
   }
