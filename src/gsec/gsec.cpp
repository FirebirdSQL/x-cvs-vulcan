...
#include "../gsec/gsec.h"
#include "../gsec/gsec_proto.h"
//#include "../jrd/jrd_pwd.h"
#include "../jrd/license.h"
#include "../jrd/svc.h"
#include "../jrd/svc_proto.h"
#include "../gsec/secur_proto.h"
#include "../gsec/gsecswi.h"
...
static void util_output(tsec* tdsec, const SCHAR*, ...);

static void data_print(tsec* tdsec, void*, const internal_user_data*, bool);
static bool get_line(tsec* tdsec, int*, SCHAR**, TEXT*, size_t);
...
static void printhelp(tsec* tdsec);
#ifndef SERVICE_THREAD
 ...
 **************************************/
	...
	try 
		...
		if (! useServices) 
			...
			dpb.putParameter(isc_dpb_gsec_attach, 1); // not 0 - yes, I'm gsec

			if (user_data->dba_user_name_entered) 
				dpb.putParameter(isc_dpb_user_name, user_data->dba_user_name);

			if (user_data->dba_password_entered) 
				dpb.putParameter(tdsec->tsec_service_gsec ?  isc_dpb_password_enc : isc_dpb_password, 
		...
		if (!tdsec->tsec_interactive) 
	...
	#ifndef SUPERCLIENT
				if (! useServices)
					ret = SECURITY_exec_line(tdsec, status, db_handle,  user_data, data_print, NULL);
				else
	#endif //SUPERCLIENT
					{
					callRemoteServiceManager(tdsec, status, sHandle, *user_data, data_print, NULL);
					ret = status[1] ? GsecMsg75 : 0;
					}
					
				if (ret) 
					{
					GSEC_print(tdsec, ret, user_data->user_name, NULL, NULL, NULL, NULL);
					if (status[1])
						GSEC_print_status(tdsec, status);
					}
				}
			}
		else {
			...
			for (;;) 
				...
				if (get_line(tdsec, &local_argc, local_argv, stuff, sizeof(stuff)))
					break;
		...
		if (db_handle) 
			...
			if (isc_detach_database(loc_status, &db_handle)) 
				GSEC_error_redirect(tdsec, loc_status, 0, NULL, NULL);
			}
	#endif //SUPERCLIENT
		...
		if (sHandle)
			...
			if (loc_status[1]) 
				GSEC_print_status(tdsec, loc_status);
			}
...
static void data_print(tsec* tdsec, void* arg, const internal_user_data* data, bool first)
{
/**************************************
 *
 *	d a t a _ p r i n t
...
#define STUFF_USER(item) fputc(item, stderr)
	...
	if (tdsec->tsec_service_gsec) 
		...
		int len = (int) strlen(data->user_name);
		...
		len = (int) strlen(data->first_name);
		...
		len = (int) strlen(data->middle_name);
		...
		if (first)	
			{
			GSEC_print(tdsec, GsecMsg26, NULL, NULL, NULL, NULL, NULL);
			GSEC_print(tdsec, GsecMsg27, NULL, NULL, NULL, NULL, NULL);
			/* msg26: "    user name                    uid   gid     full name" */
			/* msg27: "-------------------------------------------------------------------------------------------" */
			}

		util_output(tdsec, "%-31.31s %5d %5d      %s %s %s\n", data->user_name,
...
static bool get_line(tsec* tdsec, int* argc, SCHAR** argv, TEXT* stuff, size_t maxstuff)
{
/**************************************
 *
 *	g e t _ l i n e
 ...
 *
 **************************************/
	GSEC_print_partial(tdsec, GsecMsg1, NULL, NULL, NULL, NULL, NULL);
	*argc = 1;
	TEXT* cursor = stuff;
	...
	*quitflag = false;
	...
	for (--argc; argc > 0; argc--) 
			...
			user_data->middle_name_entered || user_data->last_name_entered)
		{
			...
			case 0:
				GSEC_error(tdsec, GsecMsg42, NULL, NULL, NULL, NULL, NULL);
			...
			case QUIT_OPER:
			case HELP_OPER:
				GSEC_error(tdsec, GsecMsg43, NULL, NULL, NULL, NULL, NULL);
 ...
 **************************************/
	...
	util_output(tdsec, "\n%s", "   ");
	GSEC_print(tdsec, GsecMsg45, NULL, NULL, NULL, NULL, NULL);
/* gsec utility - maintains user password database */

	util_output(tdsec, "\n%s", "   ");
	GSEC_print(tdsec, GsecMsg46, NULL, NULL, NULL, NULL, NULL);
	util_output(tdsec, "%s", "     ");
	GSEC_print_partial(tdsec, GsecMsg2, NULL, NULL, NULL, NULL, NULL);
	GSEC_print_partial(tdsec, GsecMsg82, NULL, NULL, NULL, NULL, NULL);
	GSEC_print(tdsec, GsecMsg47, NULL, NULL, NULL, NULL, NULL);
/* gsec [ <options> ... ] -<command> [ <parameter> ... ] */

	util_output(tdsec, "\n%s", "   ");
	GSEC_print(tdsec, GsecMsg48, NULL, NULL, NULL, NULL, NULL);
/* interactive usage: */

	util_output(tdsec, "%s", "     ");
	GSEC_print_partial(tdsec, GsecMsg2, NULL, NULL, NULL, NULL, NULL);
	GSEC_print(tdsec, GsecMsg82, NULL, NULL, NULL, NULL, NULL);
/* gsec [ <options> ... ] */
...
/* GSEC> <command> [ <parameter> ... ] */
	...
	util_output(tdsec, "\n%s", "   ");
	GSEC_print(tdsec, GsecMsg83, NULL, NULL, NULL, NULL, NULL);
/* available options: */

	util_output(tdsec, "%s", "     ");
	GSEC_print(tdsec, GsecMsg84, NULL, NULL, NULL, NULL, NULL);
/* -user <database administrator name> */

	util_output(tdsec, "%s", "     ");
	GSEC_print(tdsec, GsecMsg85, NULL, NULL, NULL, NULL, NULL);
/* -password <database administrator password> */

	util_output(tdsec, "%s", "     ");
	GSEC_print(tdsec, GsecMsg86, NULL, NULL, NULL, NULL, NULL);
/* -role <database administrator SQL role name> */

	util_output(tdsec, "%s", "     ");
	GSEC_print(tdsec, GsecMsg87, NULL, NULL, NULL, NULL, NULL);
/* -server <server to manage> */

	util_output(tdsec, "%s", "     ");
	GSEC_print(tdsec, GsecMsg88, NULL, NULL, NULL, NULL, NULL);
/* -z */

	util_output(tdsec, "\n%s", "   ");
	GSEC_print(tdsec, GsecMsg49, NULL, NULL, NULL, NULL, NULL);
/* available commands: */

	util_output(tdsec, "%s", "     ");
	GSEC_print(tdsec, GsecMsg50, NULL, NULL, NULL, NULL, NULL);
/* adding a new user: */

	util_output(tdsec, "%s", "       ");
	GSEC_print(tdsec, GsecMsg51, NULL, NULL, NULL, NULL, NULL);
/* add <name> [ <parameter> ... ] */

	util_output(tdsec, "%s", "     ");
	GSEC_print(tdsec, GsecMsg52, NULL, NULL, NULL, NULL, NULL);
/* deleting a current user: */

	util_output(tdsec, "%s", "       ");
	GSEC_print(tdsec, GsecMsg53, NULL, NULL, NULL, NULL, NULL);
/* delete <name> */

	util_output(tdsec, "%s", "     ");
	GSEC_print(tdsec, GsecMsg54, NULL, NULL, NULL, NULL, NULL);
/* displaying all users: */

	util_output(tdsec, "%s", "       ");
	GSEC_print(tdsec, GsecMsg55, NULL, NULL, NULL, NULL, NULL);
/* display */

	util_output(tdsec, "%s", "     ");
	GSEC_print(tdsec, GsecMsg56, NULL, NULL, NULL, NULL, NULL);
/* displaying one user: */

	util_output(tdsec, "%s", "       ");
	GSEC_print(tdsec, GsecMsg57, NULL, NULL, NULL, NULL, NULL);
/* display <name> */

	util_output(tdsec, "%s", "     ");
	GSEC_print(tdsec, GsecMsg58, NULL, NULL, NULL, NULL, NULL);
/* modifying a user's parameters: */

	util_output(tdsec, "%s", "       ");
	GSEC_print(tdsec, GsecMsg59, NULL, NULL, NULL, NULL, NULL);
/* modify <name> <parameter> [ <parameter> ... ] */

	util_output(tdsec, "%s", "     ");
	GSEC_print(tdsec, GsecMsg60, NULL, NULL, NULL, NULL, NULL);
/* help: */

	util_output(tdsec, "%s", "       ");
	GSEC_print(tdsec, GsecMsg61, NULL, NULL, NULL, NULL, NULL);
/* ? (interactive only) */

	util_output(tdsec, "%s", "       ");
	GSEC_print(tdsec, GsecMsg62, NULL, NULL, NULL, NULL, NULL);
/* help */

	util_output(tdsec, "%s", "     ");
	GSEC_print(tdsec, GsecMsg89, NULL, NULL, NULL, NULL, NULL);
/* displaying version number: */

	util_output(tdsec, "%s", "       ");
	GSEC_print(tdsec, GsecMsg90, NULL, NULL, NULL, NULL, NULL);
/* z (interactive only) */

	util_output(tdsec, "%s", "     ");
	GSEC_print(tdsec, GsecMsg63, NULL, NULL, NULL, NULL, NULL);
/* quit interactive session: */

	util_output(tdsec, "%s", "       ");
	GSEC_print(tdsec, GsecMsg64, NULL, NULL, NULL, NULL, NULL);
/* quit (interactive only) */

	util_output(tdsec, "\n%s", "   ");
	GSEC_print(tdsec, GsecMsg65, NULL, NULL, NULL, NULL, NULL);
/* available parameters: */

	util_output(tdsec, "%s", "     ");
	GSEC_print(tdsec, GsecMsg66, NULL, NULL, NULL, NULL, NULL);
/* -pw <password> */

	util_output(tdsec, "%s", "     ");
	GSEC_print(tdsec, GsecMsg67, NULL, NULL, NULL, NULL, NULL);
/* -uid <uid> */

	util_output(tdsec, "%s", "     ");
	GSEC_print(tdsec, GsecMsg68, NULL, NULL, NULL, NULL, NULL);
/* -gid <gid> */

	util_output(tdsec, "%s", "     ");
	GSEC_print(tdsec, GsecMsg71, NULL, NULL, NULL, NULL, NULL);
/* -fname <firstname> */

	util_output(tdsec, "%s", "     ");
	GSEC_print(tdsec, GsecMsg72, NULL, NULL, NULL, NULL, NULL);
/* -mname <middlename> */

	util_output(tdsec, "%s", "     ");
	GSEC_print(tdsec, GsecMsg73, NULL, NULL, NULL, NULL, NULL);
/* -lname <lastname> */
	util_output(tdsec, "\n", NULL);
}
 ...
 **************************************/
	...
	if (!get_switches(argc, argv, gsec_in_sw_table, tdsec, &quitflag)) 
		...
		GSEC_error(tdsec, GsecMsg16, NULL, NULL, NULL, NULL, NULL);
#else
		GSEC_print(tdsec, GsecMsg16, NULL, NULL, NULL, NULL, NULL);
#endif
...
void GSEC_print_status(tsec* tdsec, const ISC_STATUS* status_vector)
{
/**************************************
 *
...
static void util_output(tsec* tdsec, const SCHAR* format, ...)
{
/**************************************
 *
...
void GSEC_error_redirect(tsec* tdsec, const ISC_STATUS* status_vector,
 ...
 **************************************/
	...
	GSEC_print_status(tdsec, status_vector);
	GSEC_error(tdsec, errcode, arg1, arg2, NULL, NULL, NULL);
}

void GSEC_error(tsec* tdsec, 
				USHORT errcode,
				const TEXT* arg1,
 ...
 **************************************/
	...
	GSEC_print(tdsec, errcode, arg1, arg2, arg3, arg4, arg5);
	gsec_exit(FINI_ERROR, tdsec);
}

void GSEC_print(tsec* tdsec, 
				USHORT number,
				const TEXT* arg1,
				const TEXT* arg2, const TEXT* arg3,
				const TEXT* arg4, const TEXT* arg5)
...
void GSEC_print_partial(tsec* tdsec, 
						USHORT number,
						const TEXT* arg1,
						const TEXT* arg2, const TEXT* arg3,
						const TEXT* arg4, const TEXT* arg5)
