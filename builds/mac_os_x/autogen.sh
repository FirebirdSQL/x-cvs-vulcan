...
#
# (Note - this version for the Mac OS X has the following changes:
#   - the gnu versions of libtool and libtoolize are prefixed 
#     with the letter 'g' to distinguish them from the native
#     versions that are supplied by Apple.
#  
#

PKG_NAME=FirebirdVulcan
SRCDIR=`dirname $0`
DIE=0
...
VER=`glibtool --version|grep ' glibtool)'|sed 's/.*) \([0-9][0-9.]*\).*/\1/'`
case "$VER" in
 0* | 1\.[0-2] | 1\.[0-2][a-z]* | \
 1\.3\.[0-2] | 1\.3\.[0-2][a-z]* )
  echo
  echo "**Error**: You must have glibtool 1.3.3 or later installed."
  echo "Download the appropriate package for your distribution/OS,"
...
LIBTOOL_M4=`glibtoolize --copy --force --dry-run|grep 'You should add the contents of'|sed "s,^[^/]*\(/[^']*\).*$,\1,"`
